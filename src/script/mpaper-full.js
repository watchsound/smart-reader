/*!
 * Paper.js v0.12.15-mpaper-r9 - The Swiss Army Knife of Vector Graphics Scripting.
 * http://paperjs.org/
 *
 ...
 * All rights reserved.
 *
 * Date: Fri May 20 00:41:01 2022 +0800
 *
 ***
 *
...
var PaperScope = Base.extend({
	...
	install: function(scope, exclude) {
		var that = this;
		...
		exclude = exclude || [];
		for (var key in this)
			if (!/^_/.test(key) && this[key] && exclude.indexOf(key) < 0 ){
				if( scope[key] )
	...
	setup: function (r9topdivid, docuuid, stageProps) {
		mpaper = this;
		this.project = new Project(r9topdivid, docuuid, stageProps);
...
var UID = {
 ...
 var RU = new function() {
		 ...
		 for(var rows = 0; rows < boxRows; rows++){
			 for(var cols = 0; cols < boxCols; cols++){
				 nimage = new CroppedImage(  { image: kimage, cropX: cols * boxWidth , cropY: rows * boxHeight,
					 cropWidth: boxWidth, cropHeight: boxHeight });
				 boxList.push(nimage);
			 }
		 }
	 ...
	 function getImageStrips (kimage, horizontal){
		 ...
		 if ( horizontal ){
			 ...
			 for(var rows = 0; rows < boxRows; rows++){
				 ...
				 var nimage = new CroppedImage({image: kimage, cropX: 0, cropY: boxHeight*rows, cropWidth: boxWidth, cropHeight: adjusted_h});
				 boxList.push(nimage);
			 }
		 }
		 if( horizontal )
	 ...
	 function imgBoxEffect(layer, item, duration, transType, easing, positionFunc, isCreation, callback , sentToBackOnFinish){
		 ...
		 if ( transType < 7 || transType == 12 || transType == 11 ){
	 ...
	 function showTempObj(cly, timeline, rect, duration, offset){
	   ...
	   showpassingflash : function(params, offset){
		...
		applyingwaves : function(params, offset){
		 ...
		 MoveAlongPath: function (params, offset) {
			 var that = this;
			 this.handleTargets(params, offset, function (params, offset) {
				 that.moveAlongPath(params, offset);
			 });
		 },
...
var Point = Base.extend({
	...
	equals: function(point, epsilon) {
		epsilon = epsilon || 0;
		return this === point || point
				&& (  Math.abs(this.x - point.x ) <= epsilon && Math.abs( this.y - point.y ) <= epsilon
					|| Array.isArray(point)
						&& Math.abs( this.x - point[0] ) <= epsilon && Math.abs( this.y - point[1]) <= epsilon)
				|| false;
	},
	...
	normalize_self: function (thickness) {
		var l = this.length(), thickness = thickness || 1;
	...
	calcPoint: function (angle, dist) {
		var x = this.x + Math.cos(Point.toRadians(angle)) * dist;
		var y = this.y + Math.sin(Point.toRadians(angle)) * dist;
		return new Point(x, y);
	},
 ...
 var Constants = new function() {
	return {
	   ...
	   ' const LARGE_BUFF = 50; '  ,
		  ...
		  DEFAULT_SMALL_DOT_RADIUS : 3,
		  DEFAULT_DASH_LENGTH : 3,
		  DEFAULT_ARROW_TIP_LENGTH : 20,
		  SMALL_BUFF : 5,
		  MED_SMALL_BUFF : 10,
		  MED_LARGE_BUFF : 20,
		  LARGE_BUFF : 50,
...
var Updater = Base.extend({
	...
	initialize: function Updater(param) {
		...
		this.doneChecker = param.doneChecker;
		this.paused = false;
		this.name = this.id + '';
		this.doneJob = false;
	},
	...
	isDone : function () {
		if (this.doneChecker != null && this.doneChecker())
			return true;
		if( this.host != null && (typeof this.host.removed === 'function') && this.host.removed() )
			return true;
		return this.doneJob;
	},
	...
	update: function(event){
		var that = this;
		if (that.startAniTime > event.time || that.paused ) return;
		if (that.isDone() ){
...
var Project = PaperScopeItem.extend({
	...
	initialize: function Project(r9topdivid, docuuid, stageProps) {
		...
		if (typeof docuuid == 'undefined') {
			this._view = View.create(this,
				r9topdivid || CanvasProvider.getCanvas(1, 1));
			this.configuration = {
			...
			this._view = View.create(this,
				this.cvsdom || CanvasProvider.getCanvas(1, 1));
			this.configuration = {
				frame_width: stageProps.docwidth || 860,
				frame_height: stageProps.docheight || 480,
			}

			if (stageProps.hasBgJsAnimation) {
				this.bblayer = this.addNewLayer({ name: 'bblayer' });
				this.hasBgJsAnimation = true;
			} else {
				this.hasBgJsAnimation = false;
			}
			this._mainLayer = this.addNewLayer({ name: '' });
			this._activeLayer = this._mainLayer;
	...
	setStudio: function(s){
		this._studio = s;
		this.PageBus = this._studio.pageBus;
		this._studio.subscribe('global.message.notification', this, this.on_message, null);
		this._studio.subscribe('r9.core.study.score.report', this, this.onScoreReport, '#');
		this._studio.subscribe('r9.core.event.captionLineEvent', this, this.onR9CaptionLine, null, '#');
	},

	getStageWidth: function(){
		return this.configuration.frame_width;
	},
	getStageHeight: function(){
	   return this.configuration.frame_height;
	},
	   ...
	   if( this._activeLayer ){
			if( data && data.content ){
				var  that = this,  content =   data.content,
					astyle =   data.style || '',
					position = data.position ||  that._view.center.__add(  [0, -150] ),
				...
				var callback = {
					onEnd: function(){
						if ( !data.noTTS  ) {
							that.ttssetup(data.page||null, data.pos||0, data.pageid||'', data.prefix||'', _r9norm(content),
								data.langCode||'',  false, true, "", "", null, "", true);
						}
						...
						setTimeout(function(){
							...
							if (data.resumeAnimation)
								that.getPageBus().publish("r9.core.animation.resume",
									{ 'prefix': data.prefix || '', 'messageid': -1, 'pos': data.pos || 0 });
							if (data.jumpTimeline)
								that.getPageBus().publish('r9.core.animation.timeline',
									{ 'prefix': data.prefix || '', 'timeline': data.jumpTimeline });

						}, duration < 100 ? duration * 1000 : duration );
					}
				}
	...
	getRegistedLayer: function(obj){
		var children = this._children, len = children.length, e;
		for (var i = 0; i < len; i++) {
			e = children[i];
			if (e.getRegisteredId(obj) != undefined )
			   return e;
		}
		return undefined;
	},
	...
	getPlayerByName: function(name){
		var layer = this.getLayerByName(name) || this._activeLayer || this._mainLayer;
		return  layer._player;
	},
	...
	getLayerByName: function(name){
		...
		for(var i = 0; i < len; i++) {
			e = children[i];
			if( (!e.name && !name ) ||e.name == name || e.id == name )
				return e;
		}
		return null;
	},
	...
	hideTopLayer: function(props){
		var children = this._children, len = children.length, hasBgJs  = this.hasBgJsAnimation;
		if ( len > (hasBgJs? 3 : 1) ){
			var alayer = this.getActiveLayer();
			...
			alayer.visible = false;
			alayer.remove();
			this._activeLayer = children[len-1];
			this._activeLayer.visible = true;
	...
	getActiveLayer: function() {
		if( typeof r9 != 'undefined' ){
			var layer = this._activeLayer, children = this._children, pos;
			if (layer && layer.visible && layer.isInserted() ) return layer;
			pos = children.indexOf(layer);
			if( pos <= 0 ) pos = children.length -1;
			if( pos >= 0 ){
				this._activeLayer = children[pos];
				this._activeLayer.visible = true;
	...
	addNewLayer: function(props){
		var obj = Base.isPlainObject(props), layer;
		if( obj ){
			if( props.clips  && props.position ){
				layer = new Layer({ project: this, clips: props.clips, position: props.position, insert: true });
			}
			else {
				...
				if (w && h) {
					...
					layer.position = pos;
					layer.bounds.width = w;
					layer.bounds.height = h;
				}
			}
	...
	onR9CaptionLine: function (subject, edo, ssd) {
		var that = this, stageWidth = that.getStageWidth(), stageheight = that.getStageHeight();
		if (subject == 'r9.core.event.captionLineEvent') {
			clearTimeout(that.r9captiontimer);
			that.capline.remove();
			if (edo.caption != null && edo.caption.length > 0) {
				that.capline.position = new Point(stageWidth/2, stageheight - 25);
				that.layer().addChild(that.capline);
			}
			that._view.draw();
		}
		that.captext.remove();
		...
		if( edo.math ){
			that.captext.changeText('');
			that.captext.math = edo.math;
		} else if (edo.caption != null && edo.caption.length > 0) {
			that.captext.math = null;
			that.captext.changeText(edo.caption, edo.r9textstyle);
		}
		that.captext.position = that.capline.position;
		that._view.draw();
		that.r9captiontimer = setTimeout(function () { that.capline.remove(); that.captext.remove(); }, edo.duration * 1000);
	},
	 setVarValue : function (varid, value) {
	...
	setVariTable : function (variables) {
	...
	clearCurPageForMainline : function () {
		this.layer().removeChildren();
		if (this.playbutton) this.layer().addChild(this.playbutton);
		if (this.restartbutton) this.layer().addChild(this.restartbutton);
	...
	queryCachedLoc : function (targetid, loc) {
	...
	score_by_concept : function (conceptName) {
		for (var i = 0; i < this.score_report.length; i++) {
	...
	onScoreReport : function (subject, edo, sd) {
		if (subject == 'r9.core.study.score.report') {
			this.update_score(edo);
		}
	},

	ttssetup : function (page, pos, pageid, prefix, ttsStr, langCode, resumeAfterTTs, fromText,
		...
		if (this.usetts && this.inSpeakerMode && ttsStr.trim().length > 0) {
			...
			if (pauseAni) { this.getPageBus().publish('r9.core.animation.stop', { 'messageid': -1, 'prefix':  prefix }); }
	...
	getTopLayerSetting : function () {
		var alayer = this.getActiveLayer();
		return alayer._setting;
	},
	layer : function () {
	...
	pushNewLayer: function (name, layer, resumeAnimation, autoClose) {
		this.addNewLayer({ name: name, resumeOnClose: resumeAnimation, autoClose: autoClose});
	},

	topLayerPos : function () {
		var l = this.layer(), b = l.bounds; return { x: b.x, y: b.y };
	},
	getCurPage : function () {
	...
	getLayerPos : function (name) {
		var children = this._children, len = children.length;
		for (var i = 0; i < len; i++) {
			if (children[i].name === name || (!name && !children[i].name))
				return i;
		}
		return 0;
	},
	lys : function (name) {
		var children = this._children, len = children.length, child;
		for (var i = 0; i < len; i++) {
			child = children[i];
			if (child.name === name || (!name && !child.name))
				return child._setting;
		}
	},
	removeOverlapLayer : function (name) {
		var curLayerPos = this.getLayerPos(name), children = this._children, len = children.length;
		if (curLayerPos <= (this.hasBgJsAnimation? 2 : 0) || curLayerPos == len - 1) return;
	   for(var i = len-1; i > curLayerPos; i--){
		   var l = children[i]; this.finishLayer(l._setting);
		}
	},
	clearLayerStack : function (all) {
		var mainLayerPos = this.getLayerPos(), btm = all ?  mainLayerPos :  mainLayerPos + 1,
			children = this._children, len = children.length;;
		for (var i = len - 1; i > btm; i--){
			var l = children[i]; this.finishLayer(l._setting);
		}
	},

	finishLayerByName : function (name) {
		var layercomp = this.lys(name);
		this.finishLayer(layercomp);
	},
	finishLayer : function (layercomp) {
	...
	showMessageNode : function (dnclayer, messageid, message, anode, duration,
						...
						complete: function () {
							var tw3 = {
								...
								complete: function () {
									anode.remove();
									if (resumeAnimation)
										_this.getPageBus().publish("r9.core.animation.resume", { 'prefix': prefix, 'messageid': -1, 'pos': pos });
									if (jumpTimeline)
										_this.getPageBus().publish('r9.core.animation.timeline', { 'prefix': prefix, 'timeline': jumpTimeline });
								}
							} ;
							anime(tw3);
						}
		...
		this.getPageBus().publish('r9.core.event.studyEvent', { 'targetId': messageid, 'eventType': 'ShowMessage', 'prefix':  prefix });
	},

	showMessageBorder : function (dnclayer, messageid, otext, textstyle, math, ttstip, strokeColor, scr, scg, scb,
		...
		this.showMessageNode(dnclayer, messageid, ttstip || otext, anode, duration, resumeAnimation, jumpTimeline, noTTS, langCode,
			pos, pageid, prefix, topicId, subtopicId);

	},

	showMessageBorder2 : function (dnclayer, messageid, otext, textstyle, math, ttstip,
	...
	showMessage : function (dnclayer, messageid, message, stagewidth, stageheight, duration, resumeAnimation,
		langCode, rate, pos, pageid, prefix, topicId, subtopicId) {
		this.showMessageBorder2(dnclayer, messageid, message, null, null, null, stagewidth, stageheight, 0, 0, true, duration, resumeAnimation,
			langCode, rate, 20, pos, pageid, prefix, topicId, subtopicId);
	},
	showOptionsDialog : function (dnclayer, messageid, nodeList, strokeColor, scr, scg, scb,
		...
		nodeList.forEach(function (n) {
			...
			anode.on('click', function () {
				for (var j in nodes) {
					(function (node) {
						var tw1 = {
							target: node, opacity: 0.3, duration: 0.5, complete: function () {
								node.remove();
							}
						} ;
						anime(tw1);
				...
				if (n.concept)
					_this.getPageBus().publish('r9.core.study.score.report', n.concept);
				if (n.timeline)
					_this.getPageBus().publish('r9.core.animation.timeline', { 'timeline': n.timeline });
...
new function() {
	...
	_onFrameHandler: function(event){
		...
		var  copy = [];
		for(var i = 0; i < that._updaters.length; i++ )
			copy[i] = that._updaters[i];
		for (var key in copy){
			var updater = copy[key];
	...
	clearUpdaters: function(){
		this._updaters = [];
		this.off('frame', this._onFrameHandler);
	},
...
}, Base.each(['locked', 'draggable', 'checked', 'correct', 'rotatable',  'visible', 'blendMode', 'opacity', 'guide'],
	function(name) {
...
{}), {
	...
	firedtime: function(time){
		if( typeof time == 'undefined' ) return this._firedtime;
		this._firedtime = time;
	},
	...
	getRegistedLayer: function () {
		return this._project.getRegistedLayer(this);
	},
	addToViewIfNot: function(duration, offset){
		...
		if( !(p instanceof Layer)) {
			(this.getRegistedLayer() || this._project._activeLayer).addChild(p);
	...
	setPosition: function() {
		var p = (arguments.length == 1 && arguments[0] instanceof Point) ? arguments[0] : Point.read(arguments);
		this.translate(p.subtract(this.getPosition(true)));
		if (this.setPositionMask ){
			this.setPositionMask(p)
		}
	},

	getCameraJustedPosition:function ( ) {
	...
	getCenter: function (locType){
		var b = this.getBounds();
		switch (locType) {
			case 'top': return new Point({ x: b.getX() + b.getWidth() / 2, y: b.getY() });
			case 'bottom': return new Point({ x: b.getX() + b.getWidth() / 2, y: b.getY() + b.getHeight() });
			case 'left': return new Point({ x: b.getX(), y: b.getY() + b.getHeight() / 2 });
			case 'right': return new Point({ x: b.getX() + b.getWidth() / 2, y: b.getY() + b.getHeight() / 2 });
		}
		return new Point({
	...
	},
{
	...
	shiftCenter: function (shift) {
		var pos = this.getPosition(true);
		this.setPosition(new Point(pos.x + shift.x , pos.y + shift.y)) ;
	},
...
new function() {
	...
	_hitTestSelf: function(point, options) {
		if ((this.hitWhole || (options.fill && this.hasFill())) && this._contains(point))
	...
	moveUp: function(){
		var owner = this._getOwner(), pos = this.index;
		if (!owner || pos < 0 || owner.lastChild == this ) return;
		this.remove();
		owner.insertChild( pos+1, this);
	},
	moveDown: function () {
		var owner = this._getOwner(), pos = this.index;
		if (!owner || pos <= 0 || owner.firstChild == this) return;
		this.remove();
		owner.insertChild(pos - 1, this);
	},
	sendToBack: function() {
		var owner = this._getOwner() ;
		if( owner ) {
			var children = owner.children, count = children.length, pos = 0;
			while( pos < count ){
				if (!children[pos]._aslayerbg && !children[pos]._aspagebg)
				   break;
				pos ++;
			}
			return owner._insertItem(pos, this);
		}
		return null;
	},
	...
	_setStyles: function(ctx, param, viewMatrix) {
		var style = this._style,
			matrix = this._matrix;
		if (style.fillPatterns){
			ctx.fillStyle = ctx.createPattern(style.fillPatterns, "repeat");
	...
	imageEffect: function(duration, transType, isCreation){
		var easing = '';
		RU.imageEffect2(this.getLayer() || this.getRegistedLayer() || this._project._activeLayer, this, duration,  transType, easing, "", isCreation, null);
	},
	...
	draw: function(ctx, param, parentStrokeMatrix) {
		...
		this.drawCorrectMarker(ctx);
		this.drawCheckedMarker(ctx);
		ctx.restore();
		matrices.pop();
	...
	drawCheckedMarker: function(context){
		if (this.getChecked()) {
			var bd = this.bounds;
			context.strokeStyle = 'red';
			context.lineWidth = 1;
			context.beginPath();
			context.rect(bd.x, bd.y, bd.width, bd.height );
			context.stroke( );
		}
	},
	...
	showSelf: function( timeline, options, offset){
		var layer = this.getLayer() || this.getRegistedLayer() || this._project._activeLayer;
	...
	sockets: function () {
		if (arguments.length == 0) return this._sockets;
		this._sockets = arguments[0];
	},
	registerSocket: function (socket) {
		if (this.getSocketById(socket.id) == null) {
	...
	getSocketByLoc: function (loc) {
		var sockets = this.sockets();
		for (var i in sockets) {
			var s = sockets[i];
			if (s.hitSocket(loc)) return s;
		}
		return null;
	},
	adjustBySocket: function (socket, visited) {
		var sockets = this.sockets();
		if (sockets.length == 0) return;
		for (var i in sockets) {
			var s = sockets[i];
			if (visited.indexOf(s.id) >= 0 || !s.pin || visited.indexOf(s.pin.id) >= 0) continue;
			var ss = s.pin.getStrictMoveSocket();
			if (ss != null) {
				s.pin.setGlobalLoc(ss._getLoc(), visited);
			}
		}
	},
});
...
var LayerSetting = Base.extend({
	...
	initialize: function LayerSetting( layer, resumeAnimation, autoClose) {
		this.i2f = {};
		this.emotions = {};
		this.transpages = [];
		this.r9player = layer._player;
		this.tmpnodes = [];
		this.gnrtrids = {};
		this.layer = layer;
		this.ra = resumeAnimation || false;
	...
	destroy: function () {
		var p = this.r9player, useR9 = typeof r9 != 'undefined';
		if (p != null) {
			p.stop();
		...
		if( this.layer.visible ){
			this.layer.clearUpdaters();
			if (useR9 ){
				var name = this.layer.name ;
				this.layer.visible = false; this.layer.remove();
				r9.PageBus.cleanupTimeline(name);
			}
		}
	}
});
...
var Layer = Group.extend({
  ...
  var ViewPort = Group.extend({
	...
	_scrollHandler: function(event){
		var that = this, type = event.type, point = event.point, sp = that.getScrollPolicy(), diff = 0;
		 if( type == 'mousedown'){
			 that._prevPoint = point;
		 }
		 else if( type == 'mousedrag'){
			 diff = point.__subtract( that._prevPoint );
			 var diff_x = (sp == 'both' || sp == 'horizontal' || sp == 'h') ? diff.x : 0;
			 var diff_y = (sp == 'both' || sp == 'vertical' || sp == 'v') ? diff.y : 0;
			 that._prevPoint = point;
			 that.scroll(diff_x, diff_y);
		}
	},
	...
	setCameraPosition: function(pos, duration){
	  ...
	  addUpdater: function (update_func, duration, index, doneCallback) {
		  ...
		  if (typeof index != 'undefined' && index >= 0) {
			  this._setting._updaters.splice(index, 0, updater);
		  } else {
			  this._setting._updaters.push(updater);
		  }
		  if (this._setting._updaters.length == 1) {
			  var that = this;
			  that.on('frame', that._onFrameHandler);
		  }
		  if (this._setting._updaters.length == 0) {
			  var that = this;
			  that.off('frame', that._onFrameHandler);
		  }
		  return updater;
	  ...
	  _onFrameHandler: function (event) {
		  ...
		  if (that._paused) return;
		  var copy = [];
		  for (var i = 0; i < that._setting._updaters.length; i++)
			  copy[i] = that._setting._updaters[i];
		  for (var key in copy) {
			  var updater = copy[key];
			  updater.update(event);
		  }
	  ...
	  getUpdater: function (updaterId) {
		  for (var i = this._setting._updaters.length - 1; i >= 0; i--) {
			  if (this._setting._updaters[i].id === updaterId || this._setting._updaters[i].name == updaterId) {
				  return this._setting._updaters[i];
			  }
		  }
		  return null;
	  },
	  removeUpdaterById: function (updaterId) {
		  for (var i = this._setting._updaters.length - 1; i >= 0; i--) {
			  if (this._setting._updaters[i].id === updaterId) {
				  this._setting._updaters.splice(i, 1);
				  break;
			  }
		  }
		  if (this._setting._updaters.length == 0) {
	  ...
	  removeUpdater: function (update_func) {
		  for (var i = this._setting._updaters.length - 1; i >= 0; i--) {
			  if (this._setting._updaters[i].func === update_func) {
				  this._setting._updaters.splice(i, 1);
				  break;
			  }
		  }
...
var Raster = Item.extend({
		...
		if (!this._initialize(source,
			...
			if (object && object !== Item.NO_INSERT) {
				if (object.getContext || object.naturalHeight != null) {
					...
					if (!object.isZero()) {
						image = CanvasProvider.getCanvas(object);
					}
				} else if( object ){
					var size = Size.read(arguments);
					if (!size.isZero()) {
	...
	drawCheckedMarker: function (context) {
		if (this.getChecked()) {
			...
			context.lineWidth = 1;
			context.beginPath();
			context.rect(-sz.width/2, -sz.height/2, sz.width, sz.height);
			context.stroke();
		}
	},
	drawCorrectMarker: function (context) {
	...
	setState : function (state) {
		this.state = state;
		if (state === this.state1) { this._setImage(  this.image1 ); }
		if (state === this.state2) { this._setImage( this.image2 ); }
	},
				...
				for (var i = 0; i < size.width; i++) {
					for (var j = 0; j < size.height; j++) {
						...
						tc = curc.morphingTo(tc, progress);
							var colSize = that._raster.bounds.size .__divide( size );
...
var CroppedImage = Item.extend({
	...
	initialize: function CroppedImage(params) {
		this._initialize(params)  ;
		if (typeof params.image === 'string')
			this.raster = new Raster(params.image, params.image_pos || [0,0]);
		else {
			this.raster = params.image;
			if( params.image_pos ){
				this.raster.position = params.image_pos;
			}
		}
	...
	_draw: function(ctx, param, viewMatrix) {
		var element = this.raster.getElement();
		if (element && element.width > 0 && element.height > 0) {
			var  ebd = this.raster.bounds, ew = element.width, eh = element.height,
...
var Sprite = Item.extend({
	...
	changeState: function (animation, isPunch) {
		this.setAnimation(animation, isPunch);
	},
...
var SpriteSVG = Item.extend({
	...
	beans: true,
	initialize: function SpriteSVG(position,   data, animation, animations ) {
		this._animation = null;
		this._animations = null;
		this._data=null;
		this._cursvg=null;
		...
		this.lastAniTime = 0;
		this.startAniTime = 0;
		this._initialize(position, data, animation, animations ) ;
		var anim = this.getAnimation() ;
	...
	_onFrame: function(event){
		...
		var anim = this.getAnimation(), animsetting = this.getAnimations()[anim],
			dataindex = animsetting.dataindex;
		if (dataindex.length <= 1 ) return;
		if( that.startAniTime == 0 || event.time - that.startAniTime < animsetting.morphtime ){
			if (that.startAniTime == 0)
				that.startAniTime = event.time;
			var span =  animsetting.morphtime / Math.max(1,(dataindex.length -1)),
				timeused = (event.time - that.startAniTime),
				curindex = parseInt(timeused / span),
				progress = (timeused - curindex* span) / span;
			if( animsetting.curindex != curindex || ( animsetting.fromdata == null ) ){
				animsetting.curindex = curindex;
				var fd = that._data[dataindex[curindex]], f = this._createTempPath(fd.data ); f.remove();
				var td = that._data[dataindex[curindex + 1]], t = this._createTempPath(td.data ); t.remove();
				var from_segs = f._segments ? f._segments.length : f._children.length,
					to_segs = t._segments ? t._segments.length : t._children.length;
				if (from_segs < 20 && to_segs < 20) {
					f.insertExtraOnes(20 - from_segs);
					t.insertExtraOnes(20 - to_segs);
				} else if (from_segs < to_segs) {
					f.insertExtraOnes(to_segs - from_segs);
				} else if (from_segs > to_segs) {
					t.insertExtraOnes(from_segs - to_segs);
				}
				animsetting.fromdata = f.pathData;
				animsetting.todata = t.pathData;
				if( fd.strokeColor && td.strokeColor ){
					var fdsc = new Color(fd.strokeColor), tdsc = new Color(td.strokeColor);
					if (that.yoyoState)
						that.strokeColor = tdsc.morphingTo(fdsc, progress);
					else
						that.strokeColor = fdsc.morphingTo(tdsc, progress);
				}
				if (fd.fillColor && td.fillColor) {
					var fdsc = new Color(fd.fillColor), tdsc = new Color(td.fillColor);
					if (that.yoyoState)
						that.fillColor = tdsc.morphingTo(fdsc, progress);
					else
						that.fillColor = fdsc.morphingTo(tdsc, progress);
				}
				if (typeof fd.fillOpactiy != 'undefined' && typeof td.fillOpacity != 'undefined') {
					var ff = parseFloat(fd.fillOpactiy), tf = parseFloat(td.fillOpactiy);
					if (that.yoyoState)
						that.fillOpactiy = tf + (ff-tf)* progress ;
					else
						that.fillOpactiy = ff + (tf-ff)*progress;
				}
			}
			var f = this._createTempPath(animsetting.fromdata );
			var t = this._createTempPath(animsetting.todata );
			...
			var todata = that.yoyoState ? that._data[dataindex[dataindex.length-1]].data
			: that._data[dataindex[0]].data;
	...
	_createTempPath:function(pathDataStr){
		return this._hasMultiplePath(pathDataStr) ? new CompoundPath(pathDataStr, Item.NO_INSERT)
			:  new  Path(pathDataStr, Item.NO_INSERT);
	},

	_hasMultiplePath: function (pathDataStr){
		var c = RU.numOccuranceIn(pathDataStr, 'M') + RU.numOccuranceIn(pathDataStr, 'm');
		return c > 1;
	},
	...
	_get_cursvg: function(data){
		if (this._cursvg == null) {
			this._cursvg =  this._createTempPath(data);
	...
	setAnimation: function(animation){
		...
		var anim = this.getAnimation(),  animsetting = this.getAnimations()[anim],
		dataindex = animsetting.dataindex;
		var pathdata = this._data[dataindex[0]].data;
		animsetting.curindex = 0;
		animsetting.fromdata = null;
		animsetting.todata = null;
		this._get_cursvg(pathdata);
...
var PathItem = Item.extend({
	...
	interpolate: function(from, to, factor) {
		...
		var current = items.length,
			length = itemsTo.length, fromlen = itemsFrom.length,
			fromtail = itemsFrom[fromlen -1];
		if (current < length) {
			var ctor = isPath ? Segment : Path;
			for (var i = current; i < length; i++) {
				isPath ? this.add(new ctor()) : this.addChild(new ctor());
			}
		} else if (current > length) {
			this[isPath ? 'removeSegments' : 'removeChildren'](length, current);
		}
		for (var i = 0; i < length; i++) {
			items[i].interpolate( i<fromlen ? itemsFrom[i] : fromtail, itemsTo[i], factor);
		}
		if (isPath) {
...
var Path = PathItem.extend({
	...
	initialize: function Path(arg) {
		...
		if (arg && typeof arg.vertexDraggable != 'undefined') {
	...
	setTips: function(tips){
		...
		if( tips )
			this._tips = tips;
		this._setTips2( this.length - this._tipsOffset );
	},
	...
	_sync_tip_locs: function( ){
		var offset = this.length - this._tipsOffset,point = this.getPointAt(offset), tips = this._tips;
	...
	getLastPoint: function(){
		return this.getLastCurve().getLocationAtTime(1).point;
	},
	...
	setDataPointAt: function(pos, value){
		var segments = this._segments, length = segments.length;
		if(length ==2){
			var start = segments[0].point, end = segments[1].point;
			if( pos == 0 ) start = value; else end = value;
			var p = end.__subtract(start);
			var dp = "M" +start.x + " " + start.y + " A" +  Math.abs(p.x) + " " +  Math.abs(p.y)
				+ " 0 0 " + (p.x * p.y > 0 ? "0 " : "1 ")
				+ end.x + " " + end.y;
			this.resetPathData(dp);
			return;
		}
		this._segments[pos].point = Array.isArray(value) ? new Point(value) : value;
	},
	getDataPointAt: function(pos){
		var len = this._segments.length;
		return this._segments[ pos % len ].point;
	},
	...
	insertExtraOnes: function(numPoints){
	...
	getDataPointAtEdge: function (value, isPercentage) {
		var len =  isPercentage ? this.getLength() * value : value;
		return this.getPointAt(len);
	},
		...
		var that = this,
			...
			strokeRadius = hitStroke
					...
					: hitFill && options.tolerance > 0 || hitCurves
						? 0 : null;
		if( hitWhole ){
			var h = point.isInside(that. getBounds({ internal: false, handle: false }))
			return  h ? new HitResult('fill', this) : null;
		}
...
new function() {
	...
	return {
		_draw: function(ctx, param, viewMatrix, strokeMatrix) {
			this._draw_prepare(ctx,param, viewMatrix,strokeMatrix);
			if( this._svg ){
			...
			var dontStart = param.dontStart,
				...
				hasStroke = style.hasStroke() && (!lineType || lineType == 'Single'),
		...
		doHomotopy: function(timeline, homotopy, duration, offset, doneCallback){
			...
			var start = function(){
				if( that._svg  ) return;
					...
					if( len < 20 ){
						that._svg.insertExtraOnes(20- len);
					}
		...
		morphingTo: function (timeline, target, duration, offset, finishCallback, removeTargetAtEnd){
			if( target._class == 'Path' )
				return this._morphingTo2(timeline, target, duration, offset, finishCallback, removeTargetAtEnd);
			...
			this._morphingTo2(timeline, tc[0], duration, offset, callback, removeTargetAtEnd);
			for(var i = 1; i < tcount; i++){
				t = this.clone();
				added.push( t );
				t._morphingTo2(timeline, tc[i], duration, '==', callback, removeTargetAtEnd);
			}
		 },

		_morphingTo2: function (timeline, target, duration, offset, finishCallback, removeTargetAtEnd){
			var compound;
			if( target._class == 'CompoundPath' ){
				compound = target;
			...
			that._svgFrom = that.clone({insert:false});
			that._svgTo = target.clone({ insert: false });
			if( from_segs < 20 && to_segs < 20 ){
				that._svgFrom.insertExtraOnes( 20 - from_segs );
				that. _svgTo.insertExtraOnes( 20 - to_segs );
			} else if( from_segs < to_segs ){
				that._svgFrom.insertExtraOnes( to_segs - from_segs );
			} else if( from_segs > to_segs ){
				that._svgTo.insertExtraOnes( from_segs - to_segs );
			}
		...
		_draw_decro: function(ctx, param, viewMatrix, strokeMatrix) {
			var lineType = this.lineBorderType || '', sw = this.strokeWidth || 1, bd = this.bounds;
			if ( lineType && lineType != 'Single' ){
				this._setStyles(ctx, param, viewMatrix);
				r9_drawLineBorder(ctx, lineType, bd.x, bd.y,  bd.width, bd.height, 0, sw);
			}
		},
		_draw_prepare: function (ctx, param, viewMatrix, strokeMatrix) {
		},
...
statics: {
	...
	getHandleBounds: function(segments, closed, path, matrix, options) {
		var style = path.getStyle(),
			stroke = options && options.stroke && style.hasStroke(),
			strokePadding,
			joinPadding;
		if (stroke) {
...
var CompoundPath = PathItem.extend({
	...
	insertExtraOnes: function (numPoints) {
		if (numPoints <= 0) return;
		var that = this, newpoints = [], i = 0, children = that._children, cur_path;
		if (children.length == 0) return;
		var pos = children[children.length-1].getPointAt(0);
		while (i < numPoints) {
			cur_path = new Path({insert:false});
			cur_path.add(new Point(pos.x, pos.y));
			cur_path.add(new Point(pos.x, pos.y));
			newpoints.push(cur_path);
			this.insertChildren(children.length, cur_path);
			i++;
		}
		return newpoints;
	},
	...
	morphingTo: function (timeline, target, duration, offset, finishCallback, removeTargetAtEnd){
		...
		cc[0].morphingTo(timeline, tc[0], duration, offset, callback, removeTargetAtEnd) ;

		for(var k = 1; k < count; k++){
			try{
				cc[k].morphingTo(timeline, tc[k], duration, '==', callback, removeTargetAtEnd);
			}catch(e){
			   console.log(e);
			}
		}
	...
	_write0: function(timeline, duration, offset, create, doneCallback) {
		...
		for(var i = 1; i < count; i++){
			...
			c._write0(timeline, cdur, undefined,  create, doneCallback) ;
		}
	},
	...
	getLocationAt: function(offset){
		...
		return f == null ? null : f[0].getLocationAt(f[1]);
	},
	...
	getPointAt: function(offset){
		var f = this.getSubpathAndLocByOffset(offset);
		return f == null ? null : f[0].getPointAt(f[1]);
	},
	getLastPoint: function () {
		var children = this._children, len = children.length;
		return children[len-1].getLastCurve().getLocationAtTime(1).point;
	},

	getTangentAt: function(offset){
		var f = this.getSubpathAndLocByOffset(offset);
		return f == null ? null : f[0].getTangentAt(f[1]);
	},

	getNormalAt: function(offset){
		var f = this.getSubpathAndLocByOffset(offset);
		return f == null ? null : f[0].getNormalAt(f[1]);
	},

	getWeightedTangentAt: function(offset){
		var f = this.getSubpathAndLocByOffset(offset);
		return f == null ? null : f[0].getWeightedTangentAt(f[1]);
	},
	getWeightedNormalAt: function(offset){
		var f = this.getSubpathAndLocByOffset(offset);
		return f == null ? null : f[0].getWeightedNormalAt(f[1]);
	},
	getCurvatureAt: function(offset){
		var f = this.getSubpathAndLocByOffset(offset);
		return f == null ? null : f[0].getCurvatureAt(f[1]);
	},
	...
	_draw: function(ctx, param, viewMatrix, strokeMatrix) {
		...
		if (!param.clip) {
			this._setStyles(ctx, param, viewMatrix);
			var style = this._style, closed = this.isClosed();
...
var PathFitter = Base.extend({
 ...
 var R9Line = Path.extend({
	...
	getStart: function(){
	 ...
	 setStart: function(c){
		 var that = this, s = that.getStart();
		 if (!c.equals(s, 0.01)) {
			 var e = that.getEnd();
			 var aline = new Path.Line(c, e), length = aline.length;
			 that.getFirstSegment().point = aline.getPointAt(that.lineOffsetStart);
			 that.getLastSegment().point = aline.getPointAt( length - that.lineOffsetEnd);
			 that.getCurves();
			 aline.remove();
		 }
	 },
	 setEnd: function (c) {
		 var that = this, e = that.getEnd();
		 if (!c.equals(e, 0.01)) {
			 var s = that.getStart();
			 var aline = new Path.Line(s, c), length = aline.length;
			 that.getFirstSegment().point = aline.getPointAt(that.lineOffsetStart);
			 that.getLastSegment().point = aline.getPointAt( length - that.lineOffsetEnd);
			 that.getCurves();
			 aline.remove();
...
var Medicion = R9Line.extend({
	_class: 'Medicion',

	initialize: function Medicion(arg) {
		R9Line.apply(this, arguments);
		if (!this.fillColor) this.fillColor = this._project.getBuiltInColor('fillColor');
		if (!this.strokeColor) this.strokeColor = this._project.getBuiltInColor('strokeColor');
		if (!this.textColor) this.textColor = this._project.getBuiltInColor('textColor');
	},
	_draw: function (context, param, viewMatrix) {
		...
		len = start.getDistance(end), angle = start.calcAngle(end);
		...
		if (math != null) {
			r9_drawMathForm_paintIcon.call(this, this,
...
var Speedometer = Item.extend({
	_class: 'Speedometer',

	initialize: function Speedometer(arg) {
		this._initialize(arg);
		 this.closed = false;
		if (!this.arcAngle) this.arcAngle = 4/3 * 180;
		if (!this.numTicks) this.numTicks = 0;
		if (!this.tickLength) this.tickLength = 0;
		if (!this.needleWidth) this.needleWidth = 0;
		if (!this.needleHeight) this.needleHeight = 0;
		this.startAngle   = 90 + this.arcAngle/2;
		if (!this._velocity) this._velocity = 0;
		if (!this.handColorStr) this.handColorStr = this._project.getBuiltInColor('color1');
		if (!this.strokeColor) this.strokeColor = this._project.getBuiltInColor('strokeColor');
	},
	...
	changeValue: function (  to, duration) {
		if (duration) {
			anime({
				targets: this,
				velocity: to,
	...
	_draw: function (ctx, param, viewMatrix) {
		...
		for (var i = 0; i < numTicks; i++) {
			...
			ctx.stroke ( );
			ctx. fillStyle = this.handColorStr  ;
			ctx. font =  Math.max(16,parseInt(Math.ceil(radius / 6))) + 'px Georgia' ;
			ctx.fillText((i * 10), p.x - 5, p.y);
		}

		ctx. strokeStyle = this.handColorStr ;
		ctx.beginPath();
...
var Stack = Item.extend({
	_class: 'Stack',

	initialize: function Stack(arg) {
		this._initialize(arg);
	...
	_drawLine: function(ctx, x0,y0, x1,y1){
		ctx.moveTo(x0,y0);
		ctx.lineTo(x1,y1);
	},
	_draw: function (ctx, param, viewMatrix) {
		this._setStyles(ctx, param, viewMatrix);
		var w = this.width || 100, h = this.height || 100, queue = this.queue ;
		ctx.lineWidth = 0;
		ctx.fillRect(0,0,w,h);
		ctx.lineWidth = 3;
		if ( (!queue &&w >= h) || (queue && w <= h)) {
			ctx.beginPath();
			this._drawLine(ctx, 0, 0, w, 0);
			this._drawLine(ctx, 0, h, w, h);
			ctx.stroke();
			return;
		}
		ctx.save();
		if( queue ){
			ctx.beginPath();
			this._drawLine(ctx, 0, 0, w-h, 0);
			this._drawLine(ctx, 0, h, w -h, h);
			ctx.stroke();
			ctx.lineWidth = 1;
			ctx.setLineDash([3, 3]);
			ctx.lineDashOffset = 0;
			ctx.beginPath();
			this._drawLine(ctx, w-h, 0, w, 0);
			this._drawLine(ctx, w-h, h, w, h);
			ctx.stroke();
		} else {
			ctx.beginPath();
			this._drawLine(ctx, 0, w, 0, h);
			this._drawLine(ctx, w, w, w, h);
			this._drawLine(ctx, 0, h, w, h);
			ctx.stroke();
			ctx.lineWidth = 1;
			ctx.setLineDash([3, 3]);
			ctx.lineDashOffset = 0;
			ctx.beginPath();
			this._drawLine(ctx, 0, 0, 0, w);
			this._drawLine(ctx, w, 0, w, w);
			ctx.stroke();
		}
		ctx.restore();
	},
...
var LogBase = Base.extend({
	...
	initialize: function LogBase(base, custom_labels) {
		this.base_value = base || 1.0;
		this.custom_labels = custom_labels || [];
	},
	scale_it: function(value){
		if(  Array.isArray(value)  ){
			var that = this;
			return value.map(a => that.base_value **a  );
		}
		if( value._class == 'Point' ){
			return new Point(this.base_value ** value.x, this.base_value ** value.y  );
		}
		return this.base_value **value;
	},
	inverse_scale: function(value){
		if( value <= 0 )
			return null;
		if( Array.isArray(value) ){
			var that = this;
			return value.map(a => Math.log(arguments) / Math.log(that.base_value)   );
		}
		if( value._class == 'Point' ){
			return new Point(Math.log(value.x) / Math.log(this.base_value), Math.log(value.y) / Math.log(this.base_value) );
		}
		value = Math.log(value) / Math.log(this.base_value);
		return value;
	},
	get_custom_labels: function(){
...
var NumberLine = R9Line.extend({
	...
	initialize: function NumberLine(params) {
		...
		if( this.bindVarName && r9 ){
			this.onMouseDown = function (event) {
				var p = this.getValueByGlobalRenderPos( event.point );
				r9.varsTable[this.bindVarName] = p;
	...
	_draw_decro: function(ctx, param, viewMatrix) {
		...
		var stepLen =  this.stepLen ,
			...
			stroke = style.stroke  ;
			if( stroke && is.str(stroke) ) stroke = stroke.toCSS();
		if( stepLen <= 0 ){
			return;
		}
		...
		if (this.bindVarName && (r9 && typeof r9.varsTable[this.bindVarName] != 'undefined')){
		   var varvalue = r9.varsTable[this.bindVarName];
		   var pos = this.getGlobalRenderPosByValue(varvalue);
		   if( pos ){
			   var radius = this._style.strokeWidth * 2 || 4;
			   ctx.beginPath();
...
var CoordinateSystem = Item.extend({
	...
	initialize: function CoordinateSystem(axis_x, axis_y, grid_type,  grid_color, title) {
		this._initialize( );
		this._axis = [];
		this._axis[0] = axis_x;
		this._axis[1] = axis_y;
		...
		if( title ){
			if( typeof title == 'string' ){
				this.title = new StyledText({
					justification: 'center',
					fontSize: 20,
					fillColor: this._project.getBuiltInColor('textColor'),
					content:  title
				});
			} else {
				this.title = title;
	...
	_copyExtraAttr: function(source, excludeMatrix){
		...
		this.grid_type = source.grid_type;
	...
	_draw: function(ctx, param, viewMatrix) {
		...
		if (that.grid_type == 'grid_point') {
			ticks_on_x.forEach(function (value, index) {
				ticks_on_y.forEach(function (value2, index2) {
					ctx.beginPath();
					ctx.arc(value.x, value2.y, 2, 0, 360);
					ctx.closePath();
					ctx.fill();
				});
			});
		...
		if( this.title ){
			var bd = this.bounds;
			this.title.position = new Point( bd.x + bd.width/2, bd.y);
		}
	},
	...
	registerNode: function (dot, x_value, y_value ) {
		var that = this;
		dot.position = that.getGlobalRenderPosByValue(x_value, y_value);
	...
	registerDote: function(x_value, y_value, color, radius){
		...
		(this.getRegistedLayer() || this._project._activeLayer).addChild(dot, true);
		dot.data_value = new Point(x_value, y_value);
		this._registeredDotes.push(dot);
		return dot;
	},
  ...
  var CoordinateSystemUnit = Group.extend({
	...
	_addAllChildren:function(){
		this.removeChildren();
		var that = this, space = that.space, list = [space, space._axis[0], space._axis[1]], title = space.title;
		list.forEach(c => {
			c.remove();
			that.addChild(c);
		});;
		...
		if( title ){
			title.remove();
			that.addChild(title);
			title.position = new Point(space.position.x, space.getBounds().x -20);
		}
	},
	...
	registerFunctionCurve: function( funcCurve ){
	 ...
	 update_registered: function(){
	  ...
	  registerNode: function (dot, x_value, y_value) {
		  var dot = this.space.registerNode(dot, x_value, y_value );
		  dot.remove();
		  this.addChild(dot);
		  return dot;
	  },
...
var BarChart = CoordinateSystem.extend({
	...
	initialize: function BarChart( axis_x, axis_y, values, bar_names,  bar_name_size, bar_colors,
		 bar_width, bar_fill_opacity , duration, title ) {
		CoordinateSystem.call(this, axis_x, axis_y, false, 'black', title);
		var that = this;
		this.values = values;
...
var CategoryLineChart = CoordinateSystem.extend({
	_class: 'CategoryLineChart',

	initialize: function CategoryLineChart(axis_x, axis_y, values, line_names, line_name_size, line_colors,
		duration, title) {
		CoordinateSystem.call(this, axis_x, axis_y, false, 'black', title);
		var that = this;
		this.values = values;
		...
		this.line_name_size = line_name_size || 20;
		var bn_size = Array.isArray(values[0]) ? values[0].length : values.length;
		if (this.line_colors.length < bn_size) {
			var hue = Math.random() * 360, bc_size = this.line_colors.length;
			var b_color = bc_size > 0 ? this.line_colors[bc_size - 1] : 'pink';
			hue = (new Color(b_color).hue + 40) % 360;
			for (var i = bc_size; i < bn_size; i++) {
				var lightness = (Math.random() - 0.5) * 0.4 + 0.4;
				this.line_colors.push(new Color({ hue: hue, saturation: 1, lightness: lightness, alpha: 1 }).toCSS());
			}
		}
	...
	changeValue: function (newValues, duration, offset) {
		var that = this, firstTime = that.rects.length == 0, colors = that.line_colors, values = that.values,
			two_dim = Array.isArray(values[0]),
			axis_x = that._axis[0], axis_y = that._axis[1],
			lables = that.line_names,
			bar_clusters = values.length, x_r_width = axis_x.length,
			bar_cluster_width = x_r_width / (bar_clusters + 1), pos = 0, line_name_size = that.line_name_size,
			y_zero_render = that.getGlobalRenderPosByValue_Y(0), x_min_render = that.getGlobalRenderPosByValue_X(axis_x.x_min);
		duration = duration * 1.0 / bar_clusters || 0;
		var index = 0, timeline;
		if (duration > 0) timeline = anime.timeline({ autoplay: false });
		offset = '==';
		var y_zero_render = that.getGlobalRenderPosByValue_Y(0);
		while (pos < bar_clusters) {
			var c_center_x = x_min_render + (pos + 1) * bar_cluster_width;
			var c_data_o = values[pos], c_data_n = newValues[pos];
			if (!Array.isArray(c_data_o)) {
			...
			var numc = c_data_o.length, c_w = numc * 0, xoffset = c_center_x - c_w / 2;
			for (var i = 0; i < numc; i++) {
				var cc_data_o = c_data_o[i], cc_data_n = c_data_n[i], x_render = xoffset + i * 0;
				if (cc_data_o == cc_data_n && !firstTime) {
					index++;
					continue;
				}
				...
				var apos =  new Point(x_render, y_render);
				if (firstTime) {
					var rect = new Path.Circle({
						center: apos,
						radius: 2,
						strokeColor: color,
						fillColor: color
					...
					that.rects.push(rect);
					if (duration > 0) {
						var tween = RU.tweenPosition(rect, new Point(x_render, y_zero_render), apos, duration);
						timeline.add(tween, offset);
					}
				} else {
					var orect = that.rects[index];
					if (duration > 0) {
						var tween = RU.tweenPosition(orect, new Point(x_render, y_zero_render), apos, duration);
						timeline.add(tween, offset);
					} else {
						orect.position = apos;
					}
				...
				var y_t_render = cc_data_n > 0 ? y_render - line_name_size / 2 : y_render + line_name_size / 2;
				if (firstTime) {
					var text = new DecimalNumber({
						point: [x_render, y_t_render],
						number: cc_data_n,
	...
	_draw: function (ctx, param, viewMatrix) {
		if (!this.line_names || this.line_names.length == 0 || this.rects.length == 0) return;
		var that = this, axis_x = this._axis[0], colors = that.line_colors,
			values = that.values,
			axis_x = that._axis[0], line_name_size = that.line_name_size,
			labels = that.line_names,
			bar_clusters = values.length,  num_lines = Array.isArray(values[0]) ? values[0].length : 1,
			 x_r_width = axis_x.length,
			bar_cluster_width = x_r_width / (bar_clusters + 1), pos = 0,
			y_zero_render = that.getGlobalRenderPosByValue_Y(0),
			x_min_render = that.getGlobalRenderPosByValue_X(axis_x.x_min);

		for (var i = 0; i < num_lines; i++){
			ctx.strokeStyle = colors[i] instanceof Color ? colors[i].toCSS() : colors[i];
			ctx.beginPath();
			for( var j = 0; j < bar_clusters -1; j++){
				 var pstart = that.rects[j * num_lines + i].position;
				 var pend = that.rects[(j+1)*num_lines + i].position;
		...
		while (pos < bar_clusters) {
			...
			var c_data = values[pos], label = labels[pos];
			var cc_data = Array.isArray(c_data) ? c_data[0] : c_data;
			var y_pos_render = cc_data >= 0 ? y_zero_render + line_name_size : y_zero_render - line_name_size / 2;
			var color = Array.isArray(c_data) ? colors[0] : colors[pos];
			var label_width = ctx.measureText(label).width / 2;
			ctx.lineWidth = 1;
			ctx.strokeStyle = color;
			ctx.fillStyle = color;
			ctx.font = line_name_size + "px Courier New";
			ctx.fillText(label, c_center_x - label_width, y_pos_render);
			pos++;
		}
...
var XYLineChart = CoordinateSystem.extend({
	_class: 'XYLineChart',

	initialize: function XYLineChart(params) {
		CoordinateSystem.call(this, params.axis_x, params.axis_y, false, 'black', params.title);
		this.values = params.values;
		this.colors = params.colors || [];
		this.strokeWidth = params.strokeWidth || 1;

		this.numLines = Array.isArray(params.values[0]) ? params.values.length : 1 ;
		...
		if( this.numLines == 1 )
			this._registerLine(this.values, this.colors[0]);
		else {
			for(var i = 0; i < this.values.length; i++){
				this._registerLine(this.values[i], this.colors[i]);
			}
		}
	},
	_registerLine: function( data , color){
		var svgstr = "";
		for(var i = 0; i < data.length; i=i+2){
			svgstr +=  (i == 0 ? 'M ' : 'L ') + data[i] + ' ' + data[i+1];
		}
		this.registerFunctionCurve(
			new R9Function.SVG({
				coord_system: this,
				path_data: svgstr,
				t_range: [data[0], data[data.length-2]],
				strokeColor: color,
			})
		);
	},
...
var PieRingChart = Item.extend({
 ...
 var Table = Group.extend({
	...
	initialize: function Table(params) {
		...
		this.hide_col_lines = params.hide_col_lines || false;
		this.hide_row_lines = params.hide_row_lines || false;
	...
	createTable: function(){
		var lineColor = this.line_config.strokeColor || this._style.strokeColor ||  'black',
			...
			borderWidth = this._style.strokeWidth || strokeWidth ,
			x = this.area.x, y = this.area.y, w = this.area.width, h = this.area.height,
			data = this.data || [], col_config = this.col_config || [],
			row_config = this.row_config || [],  numcols, numrows;
		if( data.length > 0 ) {
			numcols = data[0].length, numrows = data.length;
		} else {
			if (col_config.length > 0) numcols = col_conifg.length;
			if (row_config.length > 0) numrows = row_config.length;
		}
		...
		if( this.include_outer_lines || this.background_rectangle_color != null){
			...
			if( this.include_outer_lines ){
				...
				this.bgrect.strokeWidth = borderWidth;
			} else {
		...
		if ( this.row_even && this.row_odd ) {
			var accy = y, avg_ch = h / (numrows + 1), rh, rc, rrect;
			for (var i = 0; i < numrows; i++) {
				rc = i % 2 == 0 ? this.row_even : this.row_odd;
				rh =  row_config.length > 0 ? h * row_config[i] : avg_ch;
				rrect = new Path.Rectangle([x, accy], [w, rh]);
				rrect.fillColor = rc;
				rrect.strokeWidth = 0;
				rrect.remove();
				this.addChild(rrect);
		...
		if (!this.hide_col_lines ){
			for (var i = 0; i < numcols; i++) {
				accx += col_config.length > 0 ? w * col_config[i] : avg_cw;
		...
		if( ! this.hide_row_lines ){
			var accy = y, avg_ch = h / (numrows + 1);
			for (var i = 0; i < numrows; i++) {
				accy += row_config.length > 0 ? h * row_config[i] : avg_ch;
	   ...
	   var textColor = this.text_config.strokeColor || this._style.strokeColor ||  'black',
		   ...
		   hasRowHeader = this.row_labels.length>0, hasColHeader = this.col_labels.length > 0,
		   rj = hasRowHeader?1:0, cj = hasColHeader?1:0;
		if (this.top_left_entry && hasRowHeader && hasColHeader ){
		...
		if ( hasRowHeader  ){
			for(var i = 0; i < this.row_labels.length; i++){
				var tobj = this.__addTextObj(this.row_labels[i], textColor, null, fontSize, i+cj,0);
				this.row_label_cells[i] = tobj;
			}
		}
		if ( hasColHeader ){
			for(var i = 0; i < this.col_labels.length; i++){
				var tobj = this.__addTextObj(this.col_labels[i], textColor, null, fontSize, 0, i+rj);
		...
		if( data.length > 0 ){
			for (var i = 0; i < numrows; i++) {
				this.data_cells[i] = [];
				for (var j = 0; j < numcols; j++) {
					var tobj = this.__addTextObj(data[i][j], textColor, this.cell_bg_color, fontSize, i + cj, j + rj);
	...
	_getCellSize:function( row, col){
		var x = this.area.x, y = this.area.y, w = this.area.width, h = this.area.height,
			...
			hascol = this.col_lines.length>0, hasrow = this.row_lines.length>0,
			xl, xr, yt, yb;
			if( hascol ){
				 xl = col == 0 ? x : this.col_lines[col - 1].position.x;
				 xr = col == numcols ? x + w : this.col_lines[col].position.x;
			} else {
				xl = col == 0 ? x :  x + (w / numcols) * (col-1);
				xr = col == numcols ? x + w : x + (w / numcols) * col
			}
			if( hasrow ){
				  yt = row == 0 ? y : this.row_lines[row - 1].position.y;
				  yb = row == numrows ? y + h : this.row_lines[row].position.y;
			} else {
				yt = row == 0 ? y :  y + (h/numrows) *(row-1)  ;
				yb = row == numrows ? y + h : y + (h / numrows) * row ;
			}
...
Table.inject({ statics: new function() {
	...
	function _createTable(axis_x, axis_y,  origin_render, grid_type,  grid_color, asGroup) {
	}
...
CoordinateSystem.inject({ statics: new function() {
	...
	function _createTwoAxis(params){
		...
		y_range[1] = Math.max(0, y_range[1]);
	...
	function _ByRenderAreaAndAxies(asGroup, params) {
		var  strokeColor = params.strokeColor || mpaper.project.getBuiltInColor('strokeColor')  ,
			 grid_type = params.grid_type || false,
	...
	return {
		...
		ByAxiesFullScreen: function(project, param){
			...
			return _ByAxiesFullScreen(param);
		},
		ByAxiesFullScreen2: function (param) {
			 return _ByAxiesFullScreen(param);
		},
		...
		createCategoryLineChart: function (params) {
			params.x_range = [0, 10];
			params.grid_type = 'none';
			var axies = _createTwoAxis(params);
			axies[0].include_label = false;
			axies[0].include_ticks = false;
			var chart = new CategoryLineChart(
				axies[0], axies[1],
				params.values,
				params.line_names || [],
				params.line_name_size || 24,
				params.line_colors || [],
				params.duration || 0,
				params.title
			);
		...
		createXYLineChart: function (params) {
			if (typeof params.x_range === 'undefined ')
				params.x_range = [0, 10];
			params.grid_type = 'none';
			var axies = _createTwoAxis(params);

			var chart = new XYLineChart({ axis_x: axies[0],  axis_y: axies[1],
				values: params.values,
				colors: params.colors || [],
				strokeWidth: params.stroke_width || 1,
				title: params.title
			 });
			return new CoordinateSystemUnit(chart);
		},
 ...
 var R9Function = CompoundPath.extend({
	...
	initialize: function R9Function(arg) {
		...
		if( typeof this.t_range == 'undefined')
			this.t_range = this.getSpace().getAxis_X().get_x_range();
	...
	getSpace: function(){
		if (typeof this.coord_system.space !=='undefined') return this.coord_system.space;
   ...
   _calculated_data_ranges: function(){
		var that = this, x_min = that.t_range[0], x_max = that.t_range[1], dt = that.dt;
   ...
   generate_points: function(){
	   var that = this, space = that.getSpace(), x_min, x_max, t_step = that.t_range[2];
 ...
 var SVGFunction = R9Function.extend({
	...
	generate_points: function(){
		...
		var that = this, space = that.getSpace(),  t_step = that.t_range[2] , cur_v,  cur_pos;
 ...
 var LinearFunction = R9Function.extend({
	...
	generate_points: function(){
		this.destroyContent();
		var that = this, space = that.getSpace(), x_min = that.t_range[0], x_max = that.t_range[1] ;
 ...
 var EllipseFunction = SVGFunction.extend({
	...
	generate_points: function(){
		this.destroyContent();
		var that = this, space = that.getSpace(), center = that.center, radius = that.radius ;
...
var Bubble = Path.extend({
	...
	initialize: function Bubble(arg) {
		Path.apply(this, arguments);
		this.createShapes();
	},
	_copyExtraAttr: function (source, excludeMatrix) {
	...
	createShapes: function () {
		...
		var corner = this.corner ,
			...
			rheight = this.rht || 0 ;
		if( this.cloud ){
			this.resetPathData(r9_cloud);
			var cbw = rwidth;
			var cbh = rheight;
		   this.scale(   cbw / 250.0 , cbh / 100.0 );
		} else if( this.oval ){
			var path = new Path.Ellipse({
					point: new Point(x, y),
					size: new Size(rwidth, rheight)
				 });
		   this.resetPathData(path.pathData);
		} else {
			var rectangle = new Rectangle(new Point(x,y), new Size(rwidth, rheight));
			var cornerSize = new Size(corner || 0, corner ||0);
	...
	_draw_decro: function (ctx, param, viewMatrix, strokeMatrix) {
		var  p1x = this.p1X || 0,
			...
			p2x = this.p2X || 0,
			p2y = this.p2Y || 0,
			arrowx = this.arrowX || 0,
			arrowy = this.arrowY || 0,
			cp = this.position,
			p1x  = p1x + cp.x,
			p1y = p1y + cp.y,
			p2x = p2x + cp.x,
			...
			rheight = this.rht || 0,
			btype = this.btype ||0 ,
			borderOnly = this.borderOnly||false,
		...
		if (this.bubble  > 0) {
			ey += rheight/2;
			var cx = (p1x + p2x) / 2, cy = (p1y + p2y) / 2 + rheight/2;
...
var GeomPolyFill = Path.extend({
	...
	initialize: function GeomPolyFill(arg) {
		Path.apply(this, arguments);
		this.cachedPoses = [];
	...
	createShapes: function () {
		...
		if( dirty ){
			this.resetPathData('');
			for (var i = 0; i < count; i++) {
				this.add(new Segment(cp[i].x, cp[i].y));
			}
			this.closed = true;
		}
	}
...
var TwoStateButton = Item.extend({
	...
	_draw: function (ctx, param, viewMatrix) {
		if (this._renderfig == null)
			return;
		this._setStyles(ctx, param, viewMatrix);
		this._renderfig._style = this._style.clone();
		this._renderfig._draw(ctx, param, viewMatrix);
	},
	...
	setChecked: function(param){
		this._checked = param;
		if (this.state1_name == 'selected' || this.state1_name == 'unselected' ){
		   this.setState( param ? 'selected' : 'unselected');
		} else {
			this.setState(param ? this.state1_name : this.state2_name);
		}
	},
	getChecked: function(){
	...
	changeState: function (animation ) {
		this.setState( animation );
	},
	_getBounds: function(matrix, options) {
		if( ! (this.width || this.state1_icon || this.state1_label ) )
			return new Rectangle();
		var s1icon = this.state1_icon, s2icon = this.state2_icon, s1lbel = this.state1_label, s2lbel = this.state2_label,
			w = this.width || s1icon && ( s1icon.bounds ? s1icon.bounds.width : s1icon.width ) ||  s1lbel.bounds.width,
			h = this.height || s1icon && (s1icon.bounds ? s1icon.bounds.height : s1icon.height) ||  s1lbel.bounds.height,
			rect = new Rectangle(0, 0, w, h);
		if (s1icon && (s1icon instanceof Raster  )){
			rect = rect.setCenter(0,0);
		}
		if (s1icon && (s1icon instanceof Path || s1icon instanceof CompoundPath)) {
			rect = rect.setCenter( w ,  h );
		}
		return matrix ? matrix._transformBounds(rect, rect) : rect;
	},
	isFocused: function(){
	...
	setDisabled: function(e){
		...
		if( e ){
			...
			if( this._renderfig )
				this._dis_fig.position = this._renderfig.position;
			this._renderfig = this._dis_fig;
			this._dis_fig.visible = true;

		} else {
			if( this._dis_fig ){
				this._dis_fig.visible = false;
				this._renderfig = this._get_cur_shape(this.cur_state);
				this._renderfig.visible = true;
			};
		}
		this._changed(9);
	},
	...
	setState: function(state){
		var that = this, curone = that._renderfig,  willbe = that._get_cur_shape(state);
		if (curone === willbe ) return;
		that.cur_state = state;
		if (that.state1_name == 'selected' || that.state1_name == 'unselected') {
			this._checked = that.cur_state == 'selected';
		}

		if( !curone ){
			that._renderfig = willbe;
			that._changed(9);
			return;
		}
		if (curone && that.state1_icon && that.state2_icon ){
			var bd = curone.bounds, w = bd.width, h = bd.height,
				cpos = curone.position, wpos = willbe.position;
				curone.position = that.position ;
				willbe.position = that.position ;
			that._renderfig = null;
			curone.morphingTo(null, willbe, 1.0, null, function(){
				curone.remove();
				willbe.remove();
				willbe.position = wpos;
				curone.position = cpos;

				that._renderfig = willbe;
				if( willbe instanceof Path ){
				 }
			}.bind(that), true);
		} else {
			willbe.position = that.position;
...
TwoStateButton.inject({
	statics: new function () {
		 var _checkbox_unchecked = "M 11 4 C 7.1456661 4 4 7.1456661 4 11 L 4 39 C 4 42.854334 7.1456661 46 11 46 L 39 46 C 42.854334 46 46 42.854334 46 39 L 46 11 C 46 7.1456661 42.854334 4 39 4 L 11 4 z M 11 6 L 39 6 C 41.773666 6 44 8.2263339 44 11 L 44 39 C 44 41.773666 41.773666 44 39 44 L 11 44 C 8.2263339 44 6 41.773666 6 39 L 6 11 C 6 8.2263339 8.2263339 6 11 6 z";
		 var _checkbox_checked = "M 11 4 C 7.1456661 4 4 7.1456661 4 11 L 4 39 C 4 42.854334 7.1456661 46 11 46 L 39 46 C 42.854334 46 46 42.854334 46 39 L 46 11 C 46 7.1456661 42.854334 4 39 4 L 11 4 z M 11 6 L 39 6 C 41.773666 6 44 8.2263339 44 11 L 44 39 C 44 41.773666 41.773666 44 39 44 L 11 44 C 8.2263339 44 6 41.773666 6 39 L 6 11 C 6 8.2263339 8.2263339 6 11 6 z M 36.236328 15.833984 L 22.914062 31.554688 L 14.78125 23.96875 L 13.417969 25.431641 L 23.083984 34.447266 L 37.763672 17.126953 L 36.236328 15.833984 z";
		 var _round_checkbox_checked = "M 25 3 C 12.9 3 3 12.9 3 25 C 3 37.1 12.9 47 25 47 C 37.1 47 47 37.1 47 25 C 47 20.7 45.7875 16.80625 43.6875 13.40625 L 42.3125 15 C 44.0125 18 45 21.4 45 25 C 45 36 36 45 25 45 C 14 45 5 36 5 25 C 5 14 14 5 25 5 C 30.2 5 34.9 7.0125 38.5 10.3125 L 39.8125 8.8125 C 35.9125 5.2125 30.7 3 25 3 z M 43.25 7.75 L 23.90625 30.5625 L 15.78125 22.96875 L 14.40625 24.4375 L 23.3125 32.71875 L 24.09375 33.4375 L 24.75 32.65625 L 44.75 9.03125 L 43.25 7.75 z";
		 var _round_checkbox_unchecked = "M 25 3 C 12.9 3 3 12.9 3 25 C 3 37.1 12.9 47 25 47 C 37.1 47 47 37.1 47 25 C 47 20.7 45.7875 16.80625 43.6875 13.40625 L 42.3125 15 C 44.0125 18 45 21.4 45 25 C 45 36 36 45 25 45 C 14 45 5 36 5 25 C 5 14 14 5 25 5 C 30.2 5 34.9 7.0125 38.5 10.3125 L 39.8125 8.8125 C 35.9125 5.2125 30.7 3 25 3 z";
		 var SELECT = "selected";
		 var UNSELECT = "unselected";
		 _setColor = function(params, obj){
			if( params.size )
				obj.scale(params.size / 42 );
			 obj.hitWhole = true;
			 obj.remove();
		 };
		_setupLabel = function (text, fontSize, width, height, textColor, bgColor) {
			bgColor =  bgColor || 'white'; fontSize = fontSize || 20;
			...
			if ( width) {
				label.adjustXOffsetByNewWidth( width);
			}
			if ( height) {
				label.adjustYOffsetByNewHeight( height);
			}
			label.hitWhole = true;
			label.remove();
			return label;
		...
		_button = function (params) {
			var fillColor = params.fillColor ||
				(typeof r9 === 'undefined' ? 'rgba(255,255,255,1)' : r9.getBuiltInColor('textColor')),
				strokeColor = params.strokeColor ||
					(typeof r9 === 'undefined' ? 'rgba(255,255,255,1)' : r9.getBuiltInColor('textColor'));

			if (params.type === 'checkbox') {
				params.icon1 = new CompoundPath(_checkbox_checked)
				_setColor(params, params.icon1);
				params.icon2 = new CompoundPath(_checkbox_unchecked);
				_setColor(params, params.icon2);
			}
			else if (params.type === 'radiobutton') {
				params.icon1 = new Path(_round_checkbox_checked)
				_setColor(params, params.icon1);

				params.icon2 = new Path(_round_checkbox_unchecked);
				_setColor(params, params.icon2);
			}
			else {
				if( params.icon1 ){
					if (typeof params.icon1 === 'string') {
						if (params.icon1.indexOf(' ') > 0) params.icon1 = new Path(params.icon1);
						else params.icon1 = new Raster(params.icon1);
						params.icon1.hitWhole = true;
						params.icon1.remove();
					} else {
						params.icon1 = new Raster(params.icon1);
						params.icon1.hitWhole = true;
						params.icon1.remove();
					}
				}
				else if (params.state1_name) {
					params.state1_label = _setupLabel(params.state1_name, params.fontSize, params.width || 0,
						params.height || 0, strokeColor || fillColor, params.bgColor);
				}
				if( params.icon2 ){
					if (typeof params.icon2 === 'string') {
						if (params.icon2.indexOf(' ') > 0) params.icon2 = new Path(params.icon2);
						else params.icon2 = new Raster(params.icon2);
						params.icon2.hitWhole = true;
						params.icon2.remove();
					} else {
						params.icon2 = new Raster(params.icon2);
						params.icon2.hitWhole = true;
						params.icon2.remove();
					}
				}
				else if (params.state2_name) {
					params.state2_label = _setupLabel(params.state2_name, params.fontSize, params.width || 0,
						params.height || 0, strokeColor || fillColor, params.bgColor);
				}
			}
			...
			var f = new TwoStateButton({
				state1_icon: params.icon1,
				state2_icon: params.icon2,
				state1_name: params.state1_name || SELECT,
				state2_name: params.state2_name || UNSELECT,
				state1_label: params.state1_label,
				state2_label: params.state2_label,
				cur_state: params.checked ? (params.state1_name || SELECT) : (params.state2_name || UNSELECT),
				click_func: params.click_func,
				position: params.position,
				opacity: typeof params.opacity == 'undefined' ? 1 : params.opacity,
				position: params.position,
...
var BgAnimationItem = Item.extend({
	_class: 'BgAnimationItem',
	_applyMatrix: false,
	_canApplyMatrix: false,
	_boundsOptions: { stroke: false, handle: false },
	...
	initialize: function BgAnimationItem(params) {
		this._size = new Size(r9.stageWidth, r9.stageHeight)
		this.running = false;
		this._initialize(params);
	},
	...
	start: function () {
		this.running = true;
		this.addToViewIfNot();
		this.on('frame', this._onFrame )
	},
	stop: function () {
		this.running = false;
		this.off('frame', this._onFrame )
		this.remove();
	},
	_onFrame: function (event) {
...
var PointText = TextItem.extend({
 ...
 var StyledText = TextItem.extend({
	...
	initialize: function StyledText(arg) {
		...
		this.corner= 0;
		this.borderWidth= 0;
		this.textXOffset = arg.textXOffset || 0;
		this.textYOffset = arg.textYOffset || 0;
		this.textHeight=0;
		this.textWidth=0;
		this.fixedTotalWidth = arg.fixedTotalWidth || 0;
		this.fixedTotalHeight = arg.fixedTotalHeight || 0;
		this.borderColor= '';
		this.bgColor = '';
		...
		if (this.bgColor && is.str(this.bgColor))
			this.bgColor = new Color(this.bgColor);
		if (this.borderColor && is.str(this.borderColor))
			this.borderColor = new Color(this.borderColor);
		if( this.expectTextWidth ){
			this.fontSize = this.guessFontSize(this.content, this.expectTextWidth, this.fontSize || 18);
			if( this._style && this._style.getFontSize() ){
	 ...
	 guessFontSize: function ( content, expectWidth, defaultSize){
		var  fsize = defaultSize, cw;
		 var ctx = CanvasProvider.getContext(1, 1);
		do{
			ctx.font = this._getContextFont(fsize);
			cw = ctx.measureText(content).width;
			if( Math.abs(cw - expectWidth) < 4 ){
				 return fsize;
			}
			fsize --;
		} while( fsize > 14 && cw > expectWidth );
		 fsize = defaultSize ;
		do {
			 ctx.font = this._getContextFont(fsize);
			 cw = ctx.measureText(content).width;
			 if (Math.abs(cw - expectWidth) < 2 ) {
				 return fsize;
			 }
			 fsize++;
	...
	_copyExtraAttr: function(source, excludeMatrix){
		...
		this. showunderline=  source.showunderline;
		this.useUnderline = source.useUnderline;
		...
		this.fixedTotalWidth = source.fixedTotalWidth;
		this.fixedTotalHeight = source.fixedTotalHeight;
	...
	setTextData: function(data){
	 ...
	 changeText:function(data, style){
	...
	_getBounds: function(matrix, options) {
		...
		if (this.fixedTotalWidth > 0 && this.textWidth > 0) {
			var wt = this.textWidth ;
			this.textXOffset = Math.max(0, (w - wt) / 2 - this.padding) ;
		}
		if (this.fixedTotalHeight > 0 && this.textHeight > 0){
			var ht = this.textHeight ;
			this.textYOffset = Math.max(0, (h - ht) / 2 - this.padding);
		}
   ...
   adjustYOffsetByNewHeight: function(newHeight){
	...
	getKaraokaWidth: function(){
		if (!this._karaokaWidth) this._karaokaWidth = (this.getRegistedLayer() || this._project._activeLayer).bounds.width;
	...
	getWidth: function() {
		if( this._width )
			return this._width;
		if (this.fixedTotalWidth > 0)
			return this.fixedTotalWidth;
	...
	getHeight: function() {
		if( this._height )
			return this._height;
		if (this.fixedTotalHeight > 0)
			return this.fixedTotalHeight;
	...
	_getTextSize : function(text, r9textstyle) {
		...
		if (typeof r9textstyle != "undefined") {
			if (r9textstyle.math)
				return {
					width: parseInt(  r9textstyle.math.topw || 16 ),
					height: parseInt( r9textstyle.math.toph || 16  )
				};
			if (r9textstyle.itag && r9textstyle.itagwidth)
				return {
					width: parseInt( r9textstyle.itagwidth ),
	 ...
	 _drawBackground: function (ctx, param, viewMatrix){
		var width = this.getWidth(), height = this.getHeight(),
			useBackground = this.useBackground, borderType = this.borderType,
		 borderColor = this.borderColor, bgColor = this.bgColor,
		 corner = this.corner, borderWidth = this.borderWidth, lineType = this.lineBorderType || "Single" ;
		...
		if( borderWidth ) ctx.lineWidth = borderWidth;
		 ...
		 if (borderColor) ctx.strokeStyle = is.str(borderColor) ? borderColor : borderColor.toCanvasStyle(ctx, this._matrix);
		 else if (this.strokeColor) ctx.strokeStyle = this.strokeColor.toCanvasStyle(ctx, this._matrix);

		 if ( borderType == 'Cloud' ){
			if( !this._cloudpath ){
				this._cloudpath = new Path(r9_cloud);
				this._cloudpath.remove();
			}
			this._cloudpath.fillColor = is.str(bgColor) ? bgColor : bgColor.toCanvasStyle(ctx, this._matrix);
			this._cloudpath.strokeColor = is.str(borderColor) ? borderColor : borderColor.toCanvasStyle(ctx, this._matrix);
			var scaleX = width / 250.0, scaleY = height / 100.0;
			ctx.scale(scaleX, scaleY);
			this._cloudpath._draw(ctx, param, viewMatrix);
			ctx.scale(1 / scaleX, 1 / scaleY);
			if (bgColor) ctx.fill();
			if (borderColor && borderWidth ) ctx.stroke();
		} else {
			 if (borderType == 'Normal')
				 r9_drawLineBorder(ctx, lineType, 0,0,  width, height, corner, borderWidth, bgColor);
	...
	_setTextData : function() {
		if( this.math ){
			  this._setTextDataMath();
		}
		else if ( this.r9textstyle  ){
			try{
				  this._setTextData2();
			}catch(e){
	...
	_setTextDataMath: function(){
		var w = this._width, h = this._height, math = this.math;
		this.textArr = [];
		this.line2height.push( math.toph );
		this.textHeight = math.toph;
		this.textWidth = math.topw;
	},
	_setTextData1 : function() {
		...
		for (var i = 0, max = lines.length; i < max; ++i) {
			var line = lines[i], lineWidth = this._getOneLineTextWidth(line);
	...
	_setTextData2 : function() {
	 ...
	 _sceneFuncImpl: function (ctx, param, viewMatrix, paintBgText, paintBorderOnly) {
		...
		ctx.textAlign = TextItem.LEFT;
		 ...
		 if (this.math) {
			 ...
			 var rw = this.math.w * fsize, rh = this.math.h * fsize;

			 var tXoff =  Math.max(0, (this._width - rw) / 2 );
			 var tYoff = (this._height - rh) / 2+ fsize * 0.8;
			 ctx.translate(tXoff, tYoff);
			 r9_drawMathForm_paintIcon.call(this, this, this.math, ctx, fillcolorStr,
		...
		for (n = 0; n < textArrLen; n++) {
			...
			if ( style ){
				...
				if( style.math ){
					...
					r9_drawMathForm_paintIcon.call(this, this, style.math, ctx, strokecolorStr,
						{ top: 0, left: 0, bottom: 0, right: 0 },
						0,
						0,
						fsize );
				...
				if( paintBgText) {
					...
					this._style.getFillColor().alpha = 0.3;
					ctx.fillStyle = this._style.getFillColor().toCanvasStyle(ctx, this._matrix);
				} else {
					ctx.strokeStyle = this._style.getStrokeColor().toCanvasStyle(ctx, this._matrix);
					ctx.fillStyle = this._style.getFillColor().toCanvasStyle(ctx, this._matrix);
				}
	...
	getKaraokaTextWidth: function(){
		if(! this._karaokaTextWidth )
			this._karaokaTextWidth = this._getOneLineTextWidth(this._content);
	...
	kalaokoffset: function(){
	 ...
	 _draw2: function (ctx, param, viewMatrix) {
		...
		if( (  animType == 'none' || duration <= 0 ) && (animType != 'karaoka')){
	...
	_strokeFunc : function(ctx) {
		...
		if( this.showunderline ){
			...
			ctx.beginPath();
			ctx.moveTo(0,  lh/3);
			ctx.lineTo(this._getOneLineTextWidth(this.partialText), lh/3);
			ctx.stroke();
			ctx.restore();
		}
		ctx.lineWidth = 1;
...
var LabeledDot = StyledText.extend({
	...
	_drawBackground: function (ctx, param, viewMatrix){
	   ...
	   ctx.save();
		...
		ctx.fillStyle = is.str(bgc) ? bgc : bgc.toCanvasStyle();
		ctx.strokeStyle = is.str(bc) ? bc : bc.toCanvasStyle();
			 ctx.beginPath();
...
var TextInput = StyledText.extend({
	_class: 'TextInput',
	...
	initialize: function TextInput(arg) {
		...
		if (!this.correctStyle) this.correctStyle= [] ;
		if (!this.correctMath) this.correctMath = null ;
		if (!this.wrongStr) this.wrongStr = '';
		if (!this.wrongStyle) this.wrongStyle = [] ;
		if (!this.wrongMath) this.wrongMath = null ;

		this.valid = true ;
		if (!this.pack) this.pack = 0 ;
		if (!this.math) this.math = null ;
		if (!this.linkedvarid) this.linkedvarid = null ;
		if (!this.optionMode) this.optionMode = false ;

		if (!this.textColor) this.textColor = this.strokeColor || this.fillColor || 'rgba(0,0,0,1)';
	...
	_sceneFuncImpl: function (ctx, param, viewMatrix, paintBgText, paintBorderOnly) {
		...
		ctx. font = this._getContextFont(fontSize) ;
		ctx. textBaseline = 'middle';
		if (text) {
		...
		if (!valid) {
			ctx. fillStyle = 'rgba(255,88,51,0.1)';
			ctx. strokeStyle = 'rgba(255,88,51,1)';
		}
		...
		if (math != null) {
			var textColor = this.textColor, rh =  math.h * fontSize;
			yoffset = (height - rh) / 2 + fontSize * 0.8;
			ctx.translate(0, yoffset);
			r9_drawMathForm_paintIcon.call(this, this, math, ctx, textColor,
				{ top: 0, left: 0, bottom: 0, right: 0 },
				0,
				0,
				fontSize);
			...
			this.drawTextString(ctx, text, 0, yoffset, lineHeight);
		} else if (placeholder) {
	...
	drawTextString : function (ctx, text, x, y, lineHeight) {
		...
		var strike = false;
		ctx. font = this._getContextFont() ;
		ctx.fillStyle = is.str(textColor) ? textColor : textColor.toCanvasStyle() ;
		ctx.strokeStyle = is.str(textColor) ? textColor : textColor.toCanvasStyle();
		...
		for (var i = 0; i < text.length; i++) {
			...
			var m = ctx.measureText(text.charAt(i));
			if (strike) { ctx.save(); ctx.beginPath(); ctx.moveTo(x, y - lineHeight * 4 / 5); ctx.lineTo(x + m.width, y - lineHeight * 4 / 5); ctx.closePath(); ctx.fill(); ctx.stroke(); ctx.restore(); }
			x = x + m.width;
	...
	_get_real_content: function(){
		if( this.placeholder == this._content ) return "";
		if( this.math ) return this.math.toString();
		return this._content;
	},
	validating : function () {
		var text = this._get_real_content() , answers = this.answers,
			math = this.math ;
		if (math) {
			text = math.toString();
		}
	...
	del : function () {
		var text = this._get_real_content(), feedback = _r9norm(this.correctStr ),
			math = this.math, linkedvarid = this.linkedvarid, useR9 = typeof r9 != 'undefined' ;
		if (math) {
			this._content = '';
			this.math = null;
		}
			...
			this._content = (text.substring(0, text.length - 1));
		} else {
	 ...
	 append : function (achar) {
		if (this.optionMode ) {
		 ...
		 var text = this._get_real_content() ;
		if (typeof achar === "string" || achar instanceof String) {
			if (achar == '上标') {
				achar = '<sup>';
	...
	confirm : function () {
		var that = this, text = that._get_real_content() , feedback = _r9norm( that.correctStr ),
			...
			linkedvarid = that.linkedvarid, useR9 = typeof r9 != 'undefined', pos = that.position ;
		if (linkedvarid && text) {
			if (useR9) r9.setVarValue(linkedvarid, text);
			if (useR9 ) r9.getPageBus().publish('r9.core.event.variableEvent', null);
		}
		if (feedback || fbmath || wfeedback || wfbmath) {
			if (that.validating()) {
				if (feedback || fbmath) {
					if (useR9) r9.getPageBus().publish('r9.core.message', {
						'name': 'message', 'style': fbstyle, 'math': fbmath, 'borderType': 'Cloud',
						'ttstip': '',
						'value': feedback, x: pos.getX(), y: pos.getY()
					});
				}
			} else {
				if (wfeedback || wfbmath) {
					if (useR9) r9.getPageBus().publish('r9.core.message', {
						'name': 'message', 'style': wfbstyle, 'math': wfbmath, 'borderType': 'Cloud',
						'ttstip': '',
						'value': wfeedback, x: pos.getX(), y: pos.getY()
					});
				}
			}
		}
...
var Style = Base.extend(new function() {
	...
	layerDefaults = Base.set({}, textDefaults, {
		sceneBgColor: new Color([255,255,255,0.0])
	}),
	flags = {
		strokeWidth: 193,
		strokeCap: 193,
...
Style.inject({ statics: new function() {
	var name2style =  {
		...
		'light' : {
			sceneBgColor: 'rgba( 255,255,255,0.0)',
...
new function() {
	...
	function emitMouseEvent(obj, target, type, event, point, prevPoint, pointTrack,
			stopItem) {
		var stopped = false,
			mouseEvent;
				...
				if(   dragItem == obj  ) {
				   if(  point && prevPoint ){
						if( obj instanceof Item && obj.draggable ){
							...
							if(  obj instanceof Path  && obj.rotatable ){
								var bd = obj.bounds, ct = obj.position,
									locp1 = obj.getNearestLocation(point), d1 = point.subtract(locp1.point),
									margin = locp1.curve.length;
								margin = Math.max(20, margin / 10);
							  if (d1.length < margin ){
									if( !pointTrack || pointTrack.length == 1 )
										return true;
									prevPoint = pointTrack[0], dif = point.subtract(prevPoint);
									if( dif.length <= 4)
									   return true;
									var p0 = obj.getNearestPoint(prevPoint) ,
										difonpath = p0.subtract(locp1.point), tracklength = pointTrack.length;
								  if (difonpath.length <= 1 && tracklength > 3 && dif.length > 6){
									  var mp = pointTrack[parseInt(tracklength /2)], locmp = obj.getNearestPoint(mp),
											difmp = p0.subtract(locmp);
										 if( difmp.length <=1 ){
											 obj.scale(-1, 1);
											 if (pointTrack) pointTrack = [];
										 }
										 return true;
									}
								  var pathangle = locp1.tangent.angle, moveangle = dif.angle,
									  angledif = Math.abs(pathangle - moveangle) % 360;
								  if ( (angledif < 45 || angledif > 315 ) || (angledif > 135 && angledif < 225) ) {
									  var angle = Math.min(tracklength , Math.max(1, dif.length / tracklength)) ;
									  obj.rotate(((angledif < 45 || angledif > 315) ? 1 : -1) * angle );
										  if (pointTrack) pointTrack = [];
								  }
	...
	function emitMouseEvents(view, hitItem, type, event, point, prevPoint, pointTrack) {
		view._project.removeOn(type);
		prevented = called = false;
		return (dragItem && emitMouseEvent(dragItem, null, type, event,
			point, prevPoint, pointTrack)
			|| hitItem && hitItem !== dragItem
				&& !hitItem.isDescendant(dragItem)
				&& emitMouseEvent(hitItem, null, type === 'mousedrag' ?
					'mousemove' : type, event, point, prevPoint, pointTrack, dragItem)
		...
		_handleMouseEvent: function(type, event, point) {
			...
			if ((inView || mouse.drag) && !point.equals(lastPoint)) {
				emitMouseEvents(this, hitItem, nativeMove ? type : 'mousemove',
					event, point, lastPoint, pointTrack);
				handle = true;
			}
			wasInView = inView;
			...
			lastPoint = point;
			pointTrack.push(point);
			if( pointTrack.length > 5 )
				pointTrack.splice(0,1);
			if (handle && tool) {
...
var Tool = PaperScopeItem.extend({
 ...
 var Studio = Base.extend({
	...
	setupBuiltInToolbar: function(){
	   ...
	   studio.register_tool ('Undo', '', 'Undo',  new TwoStateButton.Button({
		   state1_name: 'Undo',
	   ...
	   studio.register_tool ('Clear', '', 'Clear',  new TwoStateButton.Button({
		   state1_name: 'Clear',
		   bgColor:  mpaper.project.getBuiltInColor('bgColor2') || 'white',
		   textColor:  mpaper.project.getBuiltInColor('textColor') || 'black',
		   click_func: function(state){
...
var Toolbar = Group.extend({
	...
	register_tool: function(toolInfo){
		...
		if( toolInfo.tool instanceof Tool ){
			var button = new TwoStateButton.Button({
	...
	updateToolStatue: function(activedToolName){
		...
		for (var i = 0, l = children.length; i < l; i++) {
			var item = children[i];
			if( item._class === 'TwoStateButton.Button'){
...
function toColorObj(val){
  if(!val) return val;
  if (val._class === 'Color') return val ;
  if (is.rgb(val)) return new Color(rgbToRgba(val));
  if (is.hex(val)) return hexToRgba(val);
  if (is.hsl(val)) return hslToRgba(val);
  if (is.cnm(val)) return nameToRgba(val);
  return val;
...
function rgbToRgba(rgbValue) {
  const rgba = /rgba\((\d+,\s*[\d]+,\s*[\d]+,\s*[0|1](\.\d+)?)\)/g.exec(rgbValue);
  if (rgba) return `rgba(${rgba[1]})`;
...
function _getState(target, key, state) {
  ...
  if (state && !is.exluded(state)) {
	...
	value = value && value.clone ? value.clone({ insert: false }) : value;
	  _setProperty(target, path, current);
  } else {
	value = current && current.clone ? current.clone({ insert :false }) : current;
  }
  return value;
}
...
function normalizePropertyTweens(prop, tweenSettings) {
  ...
  if (is.arr(prop)) {
	...
	if (!isFromTo) {
	  if (!is.fnc(tweenSettings.duration)) settings.duration = tweenSettings.duration *1.0/ l;
	} else {
	  prop = {value: prop};
	}
  }
  ...
  function step(t) {
	...
	while (i < activeInstancesLength) {
	  ...
	  if (!activeInstance.paused) {
		...
		i++;
	  } else if (activeInstance.completed ){
...
function anime(params = {}) {
	...
	while (i < animationsLength) {
	  ...
	  if( anim.property == 'pfnc' &&  tween['progressFunc']  ){
		  if( typeof  tween['progressFunc'] === 'function'){
			  if (inste.callbackCxt) tween['progressFunc'].call(inste.callbackCxt, eased);
			  else tween['progressFunc'](eased);
		  } else {
  ...
  function setCallback(cb) {
	try{
	  if (inste[cb] && !inste.passThrough)
		  inste.callbackCxt ? inste[cb].call(inste.callbackCxt, inste) : inste[cb](inste);
	}catch(e){
	  console.log( cb + " exception " + e)
	  console.log( e.stack )
	}
  }
	    ...
	    if (inste.eventFunc && !inste.passThrough){
		  inste.animatables.forEach(e => {
			if(typeof inste.eventFunc === 'function'){
				inste.callbackCxt ? inste.eventFunc.call(inste.callbackCxt, inste.eventFuncParams)
  ...
  function setInstanceProgress(engineTime) {
	...
	if (!inste.began && inste.currentTime > 0) {
	  ...
	  if (inste.eventFunc && !inste.completed ){
		invokeEvent( );
		inste.paused = true;
...
function timeline(params = {}) {
  ...
  tl.addOneTimeEvt = function (node, func, stimeInSec, durationInSec) {
	this. add({
	  targets: node  ,
	  eventFunc: func,
  ...
  tl.addAni = function (node, func, stimeInSec, durationInSec, resumeAni, callback, prefix, pos, callReceiver) {
	var that = this;
	this. add({
	  targets: node  ,
	  progressFunc: function (progress) {
		if( typeof func == 'function' )
			callReceiver ? func.call(callReceiver, progress) : func(progress);
		else if( typeof node == 'object' && typeof node.make_progress == 'function' )
			node.make_progress(progress)
	  },
	  duration: durationInSec,
	  complete: function () {
		if (callback) callReceiver ? callback.call(callReceiver, progress) : callback(progress);
		if (resumeAni && r9)
		  r9.getPageBus().publish("r9.core.animation.resume", { 'prefix':  prefix, 'messageid': -1, 'pos':  pos });
	  },
	  callbackCxt: callReceiver,
	}, stimeInSec);
  };
  return tl;
}
...
var Socket = Base.extend({
	_class: 'Socket',

	initialize: function Socket(params) {
		this.id = typeof params.id == 'undefined' ? Project.idCounter() : params.id;
		this.type = params.type;
		this.direction = params.direction || "Top";
		this.locChangeType = params.locChangeType || "DirectMovable";
		...
		clone: function () {
			return new  Socket({
				type: this.type, direction: this.direction, locChangeType: this.locChangeType,
				value: this.value, node: this.node
			});
		},
		...
		canNotMoveByMouse: function () {
			var type = this.type;
			return this.locChangeType != "DirectMovable";
		},
		...
		getCircleCenterFrom3Points: function(data){
			var x1 = data[0], y1 = data[1], x2 = data[2], y2 = data[3], x3 = data[4], y3 = data[5];
			var a = x1 * x1 + y1 * y1;
			var b = x2 * x2 + y2 * y2;
			var c = x3 * x3 + y3 * y3;
			var g = (y3 - y2) * x1 + (y1 - y3) * x2 + (y2 - y1) * x3;
			var x = ((b - c) * y1 + (c - a) * y2 + (a - b) * y3) / (2 * g);
			var y = ((c - b) * x1 + (a - c) * x2 + (b - a) * x3) / (2 * g);
			return new Point({ x: x, y: y });
		},
		getGlobalLoc: function () {
			return this._getLoc();
		},
		getAdjustedGlobalLoc: function (loc) {
			if (this.canFreeMove()) return loc;
			if (this.canNotMoveByMouse()) return this._getLoc();
			var pathdata = this.node.pathData;
			return this.node.getNearestPoint(loc, pathdata);
		},
		hitSocket: function (loc) {
			var tloc = this._getLoc();
			return Math.abs(loc.x - tloc.x) < 8 && Math.abs(loc.y - tloc.y) < 8;
		},
		_getLoc: function () {
			switch (this.type) {
				case "TwoEnd":
					if (typeof this.node.getStart == 'function') {
						return this.value == 0 ? this.node.getStart( ) : this.node.getEnd( );
					}
				   return this.value == 0 ? this.node.getDataPointAt(0) : this.node.getDataPointAt(1);
				case "OnPolyJoint": return this.node.getDataPointAt(this.order);
				...
				case "OnFourSideBorder":
					...
					if (this.direction == "Bottom") return new Point({ x: bd.x + bd.width / 2, y: bd.y + bd.height });
					if (this.direction == "Left") return new Point({ x: bd.x, y: bd.y + bd.height / 2 });
					if (this.direction == "Right") return new Point({ x: bd.x + bd.width, y: bd.y + bd.height / 2 });
				case "OnLine":
					var p =  this.locChangeType == 'Percentage';
					return this.node.getDataPointAtEdge(this.value, p);
				case "OnPolyFree":
					var d1 = this.node.getDataPointAt(this.order);
					var d2 = this.node.getDataPointAt(this.order + 1);
					return d1.interpolate(d2, this.value);
				case "OnPath":
				case "OnOval":
					return this.node.getDataPointAtEdge(this.value, false);
			}
		},
		merge: function (mergedSocket) {
			if (!this.pin && !mergedSocket.pin) {
				this.pin = new SocketPin();
				this.pin.sockets.push(this);
				mergedSocket.pin = this.pin;
				this.pin.sockets.push(mergedSocket);
			}
			else if (this.pin == meredSocket.pin);
			else if (this.pin && mergedSocket.pin) {
				this.pin.merge(mergedSocket.pin);
			} else if (this.pin) {
				mergedSocket.pin = this.pin;
				this.pin.sockets.push(mergedSocket);
			} else if (mergedSocket.pin) {
				this.pin = mergedSocket.pin;
				this.pin.sockets.push(this);
			}
			return this.pin;
		}
...
var SocketPin = Base.extend({
	_class: 'SocketPin',

	initialize: function SocketPin(id) {
		this.id = typeof id == 'undefined' ? Project.idCounter() : id;
		this.sockets = [];
	},
	add: function (socket) {
		this.sockets.push(socket);
	...
	canFreeMove: function () {
		return this.getStrictMoveSocket() == null;
	},
	canNotMoveByMouse: function () {
		var s = this.getStrictMoveSocket();
		return s != null && s.canNotMoveByMouse();
	},
	getStrictMoveSocket: function () {
		for (var i in this.sockets) {
			if (!this.sockets[i].canFreeMove()) {
				return this.sockets[i];
			}
		}
		return null;
	},
	syncWithUIChange: function (visited) {
		if (this.sockets.length == 0 || this.canNotMoveByMouse()) return;
		var s = this.getStrictMoveSocket();
		if (s == null) {
			s = this.getSocketByType('ShapeCenter');
			if (s == null) s = this.sockets[0];
		}
		var loc = s._getLoc();
		this.setGlobalLoc(loc, visited);
	},
	getSocketByType: function (type) {
		for (var i in this.sockets) {
	...
	getAdjustedGlobalLoc: function (loc) {
		var s = this.getStrictMoveSocket();
		return s == null ? loc : s.getAdjusetdGlobalLoc(loc);
	},
	setGlobalLoc: function (loc, visited) {
		visited = visited || [];
		if (visited.indexOf(this.id + "_pin") >= 0) return;
		visited.push(this.id + "_pin");
		var tloc = this.sockets[0]._getLoc();
		for (var i in this.sockets) {
			var s = this.sockets[i];
			if (visited.indexOf(s.id) >= 0) continue;
			visited.push(s.id);
			s.setGlobalLoc(loc, visited);
			if (s.canAffectOthersOnNode())
				s.node.adjustBySocket(s, visited);
		}
	},
	merge: function (mergedPin) {
		for (var i in mergedPin.sockets) {
			var s = mergedPin.sockets[i];
			if (this.sockets.indexOf(s) < 0) this.sockets.push(s);
			s.pin = this;
		}
	},
	valid: function () {
		for (var i = this.sockets.length - 1; i >= 0; i--) {
			var s = this.sockets[i];
			if (!s.node.parent) this.sockets.splice(i, 1);
		}
...
var R9TimlinePlayer = Base.extend({
	...
	setupPlan : function (rplan) {
		this.r9pplan = rplan;
	},
	checkAnimationPlan : function () {
		var that = this, useR9 = typeof r9 != 'undefined';
		if (!that.r9pplan || that.r9pplan.length == 0) return false;
		try {
			var aplan = that.r9pplan[0];
			if (aplan.end >= 0 && aplan.end <= that.currentStep) {
				var jumploc = r9parseBookmark(aplan.jumpFrom, true);
				var jumpTimeline = r9parseBookmark(aplan.jumpFrom, false);
				var page = that.getCurPage();
				if (jumploc > 0 || that.r9pplan.length > 1) {
					...
					if (jumploc > 0) {
						if (useR9 ) r9.PageBus.publish('r9.core.animation.hotphase', { 'hotphase_pos': jumploc, 'timeline_name': jumpTimeline, 'pplan': [], 'destory_it': true });
						return true;
					} else {
						that.r9pplan.splice(0, 1);
						if ( useR9 ){
							if( jumpTimeline == that.prefix() ){
								that.initialStartFrom(that.r9pplan[0].start);
							} else {
								r9.PageBus.publish('r9.core.animation.hotphase', { 'hotphase_pos': 0, 'pplan': that.r9pplan, 'destory_it': true });
							}
						}
						return true;
					}
	...
	prefix: function(){
		return this.cly.name || '';
	},
	isMainTimeline : function () {
	...
	nextStep : function(){
		...
		if (this.currentStep == this.chain.length) {
			...
			if (useR9 ) r9.PageBus.publish("r9.core.animation.finish", { 'targetId': 'curFrameIndex_' + this.currentStep, 'prefix': prefix });
			this.stop();
		} else {
	...
	getCurPage : function(){
		return this.chain[this.currentStep];
	},
	reset : function(){
		var that = this;
	...
	resume : function(callback){
		...
		else if (this.state === 'next') this.nextStep();
		else this.leavingPage();
		this.state = '';
		if (typeof callback === 'function') callback();
	},
	stop : function(){
	...
	restartFrom : function(page, phasePosition){
		...
		if ( p  ){
			page.betweenPageInSec = p.betweenPageInSec;
...
var Page = Base.extend({
	_class: 'Page',
	This: this,
	initialize: function Page(cly,   pos, block, pageid, betweenPageInSec, stayTimeInSec){
		this.cly = typeof cly._setting == 'undefined' ? cly : cly._setting;
		this.blockAnimation = block || false;
		this.staytime = stayTimeInSec || 0;
		this.pos = pos || 0;
		this.pageid = pageid || '';
		this.betweenPageInSec = betweenPageInSec || 0;
		this.pinList = [];
		...
		if( !mpaper.settings.r9mode)
			this.layer().addPage(this);
		var that = this;
		this.layer().addUpdater(function(){
			that.syncNodesByPins();
		});
	},
	_on_anim_end:function(){
		if (this.anim_ended ) return;
		this.anim_ended = true;
		if (this.pagestay) this.pagestay();
		var layer = this.layer();
		layer.locked = false;
		if (this.blockAnimation) layer._player.stop();
		else layer._player.leavingPage();
	...
	layer: function(){
		return this.cly.layer;
	},

	remove: function(){
		this.layer().removePage( this );
		this.pos = -1;
	},
	add_func_to_tl: function(func, offset){
		...
		if (that.anim_ended || !that.ptimer.paused){
			anime(config, offset);
		} else {
			that.ptimer.add(config, offset);
		}
	},
	add_to_tl: function(param, offset){
		if (this.anim_ended || !this.ptimer.paused) {
			anime(param, offset);
		} else {
			this.ptimer.add(param, offset);
		}
	},
	add: function (param, offset) {
		this.add_to_tl(param, offset);
	},
	...
	addOneTimeEvt0: function (node, func, stimeInSec, durationInSec) {
		var tline = anime.timeline({ autoplay: true });
		tline.addOneTimeEvt(node, func, stimeInSec, durationInSec);
	},
	addAni : function (node, func, stimeInSec, durationInSec,   resumeAni, callback, callbackReceiver ) {
		var tline = this.runnableTimeline();
		tline.addAni(node, func, stimeInSec, durationInSec, resumeAni, callback, this.prefix(), this.pos, callbackReceiver);
	},
	...
	pagesetup: function(){
		...
		this.created = true;
		var layer = this.layer();
		this.setup2(this, layer, this.ptimer);
		if( this.staytime > 0 ){
	...
	getPinByName : function (pinName) {
		var pinlist = this. pinList;
		if (!pinlist[pinName])
			pinlist[pinName] = new  SocketPin(pinName);
		return pinlist[pinName];
	},
	registerPin : function (pinName, socket) {
		var pin = this.getPinByName(pinName);
		pin.sockets.push(socket);
		socket.pin = pin;
		return pin;
	},
	syncNodesByPins : function () {
		var visited = [], pinlist = this. pinList;
...
(function (window) {
	...
	window. getFontHeight = function(ctx, weight, fsize, fname) {
	...
	window. r9_formatTicker = function(amount) {
	...
	window.r9_drawLineBorder = function (ctx, lineType, x, y, width, height, corner, borderWidth, fill) {
		ctx.translate(x, y);
		if (corner > 0) {
			r9_drawRounded(ctx, 0, 0, width, height, corner, borderWidth, fill);
		} else {
			var c = ctx , t = lineType;
			if (t == 'Single') {
				c.beginPath();
				c.rect(0, 0, width, height);
				c.closePath();  if( borderWidth ) c.stroke(); if(fill) c.fill();
			} if (t == 'Double') {
				c.beginPath();
				c.rect(0, 0, width, height);
				c.closePath(); c.stroke(); if (fill) c.fill();
				var sw = borderWidth;
				c.beginPath();
				...
				c.beginPath(); c.rect(0, height - dw * 2, dw * 2, dw * 2); c.closePath(); c.stroke();
			} else if (t == 'DoubleSqu') {
				if (fill) {
					c.beginPath();
					c.rect(0, 0, width, height);
	...
	window.r9_drawRounded = function (ctx, x, y, w, h, r, borderWidth, fill) {
		var rawctx = ctx;
		if (w < 2 * r) r = w / 2;
		if (h < 2 * r) r = h / 2;
		rawctx.beginPath();
		...
		rawctx.closePath();
		if (borderWidth) rawctx.stroke();
		if( fill ) rawctx.fill();
	};
	window. r9_drawLinePat = function(points, length, progress, duration, lastPoint) {
	...
	window. r9getURLPara = function(name) {
		var v = r9getURLPara2(name);
		return v == null ? "" : v;
	};
	window. r9getURLPara2 = function(name) {
		return decodeURIComponent((new RegExp('[?|&]' + name + '=' + '([^&;]+?)(&|#|;|$)').exec(location.search) || [null, ''])[1].replace(/\+/g, '%20')) || null;
	};

	window. normalizeString = function(content) {
		if (content)
			return content.replace(/r9newline/g, '\n').replace(/r9apostrophe/g, "'").replace(/r9backslash/g, "\\").replace(/&lt;/g, "<").replace(/&gt;/g, ">");
		else
			return content;
	};
	window. _r9norm = function(content) {
		return normalizeString(content);
	};

	window. r9figure = function(properties) {
		var newpropertis = {};
	...
	window. fromShortTag = function(tag) {
		if ("sw" == tag)
			return "strokeWidth";
		if ("sr" == tag)
			return "strokeRed";
	...
	window. r9parseBookmark = function(bookmark, loc) {
...
; (function (window) {
	var r9RAFtime = 0;
	var r9RAFPx = 'webkit moz ms o'.split(' ');
...
window. openExtraR9Browser = function (url) {
...
window. R9RequestRegister = function() {
	this.rqcounts = 0;
	this.timer = null;
}
...
window. speakText_notts = function (pos, pageid, prefix, text, langCode, resumeAfterTTs, rate, topicId, subtopic, callback, ttsNoBlkAni) {
	var extime = 0;
...
window. speakText = function (pos, page, prefix, text, langCode, resumeAfterTTs, rate, topicId, subtopic, callback, ttsNoBlkAni) {
	...
	if (usetts) {
		...
		langCode = ((langCode && langCode.startsWith('zh')) ? 'zh-CN' : langCode);
...
window. startSpeakInPage = function (page, silent) {
	if (page) {
		page.mdstarted = true;
	}
	if (!silent)
...
window. playR9AudioFile2 = function (oAudio, blockAnimation, page) {
	try {
		var firstTime = true;
		function _wait(count) {
			if (oAudio.played) {
...
window. exitVideoPaneIOS = function () {
...
window. createVideoPaneIOS  = function(videoContainerId, sourceUrl) {
...
window. createVideoPane = function(sourceUrl) {
	var node = document.getElementById("video_container");
	if(!node) return;
	if (node.style.removeProperty) {
		node.style.removeProperty("display");
...
;(function (window) {
	...
	var paintIcon = function (fig, box, ctx, color, insets, x, y, size) {
		if (color && !is.str(color)) color = color.toCSS();
	...
	window.r9_drawMathForm_paintIcon = paintIcon;
})(window);
...
window. r9twnrm = function (obj, dur, etype, easingname) {
	...
	if (easingname === 'Shrinking') {
		r9shrinktwm(obj, 0, function () { r9twnrm(tweens, obj, dur, etype, 'Linear') });
		return;
	}
	var bd = obj.bounds, x = bd.x, y = bd.y;
	if (typeof r9 == 'undefined') return;
	var page = r9.getCurPage();
	if (etype == 13) etype = Math.floor(Math.random() * 10);
	if (etype == 0)
		page.add({
			target: obj, opacity: 0.8, strokeAlpha: 0.5, complete: function () {
				page.add({ target: obj, opacity: 0, strokeAlpha: 0, complete: function () { obj.remove(); }, duration: dur * 0.8 }) ;
			}, duration: dur * 0.2
		}) ;
	else if (etype == 5 || etype == 6) {
		var x = etype == 1 ? (x || -500) : (x ? 1000 - x : 1000);
		page.add({
			target: obj, x: x,
			complete: function () { obj.remove(); }, duration: dur, easing: easingname
		}) ;
	}
	else if (etype == 3 || etype == 4) {
		var y = etype == 3 ? (x || -500) : (x ? 1000 - x : 1000);
		page.add({
			target: obj, y: y,
			complete: function () { obj.remove(); }, duration: dur, easing: easingname
		}) ;
		...
		page.add({
			target: obj, opacity: 0.1,
			complete: function () { obj.remove(); }, duration: dur, easing: easingname
		}) ;
	}
}

window. tween_r9figure = function (node, properties) {
	if (typeof r9 == 'undefined') return;
	var newpropertis = node ?  { target: node } : {};
	for (var key in properties) {
		newpropertis[fromShortTag(key)] = properties[key];
	}
	return newpropertis ;
...
window. r9tween = function (tweens, properties, useobj) {
	if (!useobj) {
		var newpropertis = {};
		for (var key in properties) {
		...
		tweens.push(page.add(newpropertis));
	}
	else
		tweens.push(properties);
}

window. r9tween2 = function (tweens, obj, useobj) {
	if (obj)
		r9tween(tweens, { target: obj, opa: 1, dur: 1 }, useobj);
}
